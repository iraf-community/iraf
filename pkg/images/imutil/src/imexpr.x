...
define	IO_OP		($1+11)			# pointer to evvexpr operand
...
define	LEN_IMEXPR	(23+LEN_IMOPERAND*MAX_OPERANDS)
...
define	IE_IMOP		($1+23+(($2)-1)*LEN_IMOPERAND)	# image operand array
...
image_
	...
	repeat {
		...
		if (O_LEN(out) == 0) {
		    ...
		    switch (O_TYPE(out)) {
		    case TY_BOOL:
			call achtis (Memi[ip], Mems[op], npix)
