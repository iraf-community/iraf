...
begin
	...
	# the input list will be exhausted by ICOMBINE.

	while (imtgetim (ilist, Memc[input], SZ_FNAME) != EOF) {
	    if (imtgetim (olist, Memc[output], SZ_FNAME) == EOF) {
		if (project) {
		    call eprintf ("ICOMBINE: No output image for %s\n")
			call pargstr (Memc[input])
		    next
		} else {
		    call eprintf ("ICOMBINE: No output image\n")
			call pargstr (Memc[input])
		    break
		}
	    }
		...
		Memc[plfile], Memc[sigma], Memc[logfile]))
		call erract (EA_WARN)
	}
...
procedure icombine (list, input, output, plfile, sigma, logfile)
...
int	i, j, nimages, intype, bufsize, maxsize, memory, oldsize
pointer	sp, in, out[3], offsets

char	clgetc()
int	imtlen(), imtgetim(), getdatatype()
int	begmem(), errcode(), open(), ty_max(), sizeof()
pointer	immap(), ic_plfile()
errchk	immap, ic_plfile, ic_setout
...
retry_
	...
	out[1] = immap (output, NEW_COPY, Memi[in])
	...
	if (plfile[1] != EOS)
	    out[2] = ic_plfile (plfile, NEW_COPY, out[1])
	else
	    out[2] = NULL

	# Open the sigma image if given.
	if (sigma[1] != EOS) {
	    out[3] = immap (sigma, NEW_COPY, out[1])
