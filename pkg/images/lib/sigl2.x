...
begin
	...
	do i = 1, SI_MAXDIM {
	    ...
	    if (INTVAL(start) && INTVAL(tau[i]) && (p1[i] != p2[i]))
		SI_INTERP(si,i) = NO
	    else
		SI_INTERP(si,i) = YES
...
begin
	...
	do i = 1, 2 {
		...
		if (SI_INTERP(si,1) == NO)
		    ...
		    if (x1 == x2)
			call amovks (Mems[rawline], Mems[SI_BUF(si,i)], npix)
		    else
...
begin
	...
	if (ybavg > 1)
	    call adivks (Mems[b], short(nlines_in_sum), Mems[a], nblks_x)
...
begin
	...
	do i = 1, 2 {
		...
		if (SI_INTERP(si,1) == NO)
		    ...
		    if (x1 == x2)
			call amovkr (Memr[rawline], Memr[SI_BUF(si,i)], npix)
		    else
