...
int	xxorder, xyorder, xxterms, yxorder, yyorder, yxterms
...
begin
	...
	if (gd != NULL) {
	    ...
	    call geo_fitr (fit, sx1, sy1, sx2, sy2, Memr[xref], Memr[yref],
		Memr[xin], Memr[yin], Memr[wts], npts, Memc[xerrmsg],
...
begin
	...
	if (gd != NULL) {
		...
		Memd[xin], Memd[yin], Memd[wts], npts, Memc[xerrmsg],
		Memc[yerrmsg], SZ_LINE)
	    if (verbose && res != STDOUT) {
		call printf ("%s  %s\n")
		    call pargstr (Memc[xerrmsg])
