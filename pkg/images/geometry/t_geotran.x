...
pointer	sp, geo, sx1, sy1, sx2, sy2, in, out
...
# GEOSET -- Procedure to set picture dependent parameters
...
# GEODFORMAT -- Procedure to get the coordinate transformation from
# a database file.
...
begin
	# map database and locate record
	...
	# set output image format parameters
	call geodout (in, out, geo, sx1, sy1)

	# adjust linear part of fit
	...
	if (ncoeff > 0 && (GT_GEOMODE(geo) == GT_GEOMETRIC || GT_GEOMODE(geo) ==
	    ...
	    # set higher order surface pointers to null
	    sx2 = NULL
	    sy2 = NULL
	}
...
# GEOFORMAT -- Procedure to format the output picture when there is no
# database file
...
begin
	...
	# set up x reference coordinate limits
...
# GEODBOUT --  Set output image format
...
begin
	# set reference coordinate limits
...
# GEODSHIFT -- Procedure to adjust the shift
...
# GEOSHIFT -- Procedure to adjust the shift
...
# GEOGSSHIFT -- Procedure to shift coefficients
...
# GEODCOEFF -- Procedure to transform the coefficients
