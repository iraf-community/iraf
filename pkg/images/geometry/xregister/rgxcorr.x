...
begin
	...
	if (rg_xstatr (xc, APODIZE) > 0.0) {
	    call rg_xapodize (Memr[rbuf], nrcols, nrlines, rg_xstatr (xc,
		APODIZE), YES)
	    call rg_xapodize (Memr[ibuf], ncols, nlines, rg_xstatr (xc,
	        APODIZE), YES)
	}
...
# RG_XAPODIZE -- Apply a cosine bell to the data. The operation can be
# performed in place

procedure rg_xapodize (data, nx, ny, apodize, forward)
