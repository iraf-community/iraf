...
# RG_CNTR -- Produce a contour plot of a subrasteer.

procedure rg_cntr (gp, title, data, ncols, nlines)

pointer	gp			#I pointer to graphics stream
char	title[ARB]		#I the plot title
...
begin
	...
	if (perimeter) {
	    ...
	    call strcpy ("X-Correlation Function", Memc[label], SZ_LINE)
	    ...
	    call gtext (gp, (wx1 + wx2) / 2.0, ybot, title, "h=c;v=b;s=.5")
