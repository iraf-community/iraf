...
define	HELPFILE	"images$geometry/xregister/xregister.key"
define	OHELPFILE	"images$geometry/xregister/oxregister.key"
...
begin
	...
	if (rg_xstati (xc, NREGIONS) <= 0) {
	    ...
	    call rg_pxwrec (xc, rg_xstati (xc, CREGION))
	    newdata = NO
	    newcross = NO
	    newcenter = NO
	    newplot = NO
...
begin
	...
	if (ywindow == 1) {
	    ...
	    switch (plottype) {
	    case XC_PCONTOUR:
	        call rg_cntr (gd, Memc[title], Memr[rg_xstatp (xc, XCOR)],
	            xwindow, ywindow)
	    case XC_PLINE:
	        call sprintf (Memc[title+strlen(Memc[title])], SZ_LINE,
	            "\nX-Correlation Function: line %d")
