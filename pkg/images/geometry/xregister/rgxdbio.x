...
begin
	...
	do i = 1, nregions {
		...
		if (dformat == YES)
	            call rg_dbshifts (db, Memi[prc1+i-1], Memi[prc2+i-1],
...
# RG_DBSHIFTS -- Write out the reference image section, input image
# section and x and y shifts for each region.

procedure rg_dbshifts (db, rc1, rc2, rl1, rl2, c1, c2, l1, l2, xshift, yshift)
...
# RG_DBTSHIFTS -- Write the average shifts to the shifts database.

procedure rg_dbtshifts (db, xc)
...
# RG_PXWREC -- Print the computed shift for a region.

procedure rg_pxwrec (xc, i)
