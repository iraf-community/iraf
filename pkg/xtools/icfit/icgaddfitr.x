...
procedure icg_addfitr (ic, gp, cursor, gt, cv, x, y, wts, npts, nmax)
...
real	x[nmax]			# Ordinates
real	y[nmax]			# Abscissas
...
begin
	...
	call smark (sp)
	call salloc (userwts, nmax, TY_REAL)
	...
	repeat {
		...
		if (cmd[1] == '/')
		    ...
		    call icg_colonr (ic, cmd, newgraph, gp, gt, cv, x, y, wts,
			npts)
		...
		if (i == 1)
		    call icg_insertr (wx, wy, 1., x, y, wts,
			Memr[userwts], npts)
		else if (i == 2)
		    call icg_insertr (wy, wx, 1., x, y, wts,
			Memr[userwts], npts)
		...
		i = icg_nearestr (ic, gp, gt, cv, x, y, npts, wx, wy)
		...
		call icg_deleter (ic, gp, gt, cv, x, y, wts, Memr[userwts],
		    npts, wx, wy)

	    case 'f': # Fit the function and reset the flags.
dofit_		iferr {
		    call ic_fitr (ic, cv, x, y, wts, npts, IC_NEWX(ic),
		...
		call icg_sampler (ic, gp, gt, x, npts, 0)
		...
		call icg_undeleter (ic, gp, gt, cv, x, y, wts, Memr[userwts],
		    npts, wx, wy)
		...
		if (IC_AXES(ic, IC_GKEY(ic), 2) != axes[2]) {
	    	...
	    	call icg_graphr (ic, gp, gt, cv, x, y, wts, npts)
		newgraph = NO
	    }
