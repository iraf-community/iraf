...
procedure icg_addfitd (ic, gp, cursor, gt, cv, x, y, wts, npts, nmax)
...
double	x[nmax]			# Ordinates
double	y[nmax]			# Abscissas
...
begin
	...
	call smark (sp)
	call salloc (userwts, nmax, TY_DOUBLE)
	...
	repeat {
		...
		if (i == 1)
		    call icg_insertd (wx, wy, 1., x, y, wts,
			Memd[userwts], npts)
		else if (i == 2)
		    call icg_insertd (wy, wx, 1., x, y, wts,
			Memd[userwts], npts)
