...
begin
	if (DEBUG) { call eprintf ("ex_no_interleave:\n") 
		...
		call pargi(EX_NEXPR(ex));call pargl(EX_OCOLS(ex))
...
begin
	...
	do i = 1, EX_NLINES(ex) {
	    ...
	    call ex_merge_pixels (Memp[pp], EX_NEXPR(ex), npix, outtype, 
		outptr, totpix)
...
#		if (IO_ISIM(op) == NO)
	    ...
	    do j = 1, EX_NEXPR(ex) {
	        if (outtype != TY_CHAR && outtype != TY_UBYTE)
		    call mfree (Memp[pp+j-1], outtype)
		else
		    call mfree (Memp[pp+j-1], TY_CHAR)
	    }
...
begin
	...
	do i = 1, nptrs {
		...
		sz_val = IM_LEN(im,1)
		call amovs (Mems[data], Mems[IO_DATA(op)], sz_val)
	        IO_TYPE(op) = TY_SHORT
		...
		sz_val = IM_LEN(im,1)
		call amovi (Memi[data], Memi[IO_DATA(op)], sz_val)
	        IO_TYPE(op) = TY_INT
		...
		sz_val = IM_LEN(im,1)
		call amovd (Memd[data], Memd[IO_DATA(op)], sz_val)
...
size_t	npix				#i number of pixels to write
...
begin
	# Write binary output.
        ...
        case TY_SHORT:
	    # arg2: incompatible pointer (ilrd)
            call write (fd, Mems[pix], npix * SZ_SHORT/SZ_CHAR)
...
size_t	npix				#i number of pixels to write
long	line				#i current output line number
	...
	if (merged == YES && nbands > 1) {
		...
		k = 0
		    ...
		    case TY_CHAR, TY_SHORT, TY_USHORT:
		        call fprintf (fd, "%d\n")
		            call pargs (Mems[data+((ii-1)*npix+i)-1])
        	    case TY_INT:
		        call fprintf (fd, "%d\n")
		            call pargi (Memi[data+((ii-1)*npix+i)-1])
        	    case TY_LONG:
		        call fprintf (fd, "%d\n")
		            call pargl (Meml[data+((ii-1)*npix+i)-1])
        	    case TY_REAL:
		        call fprintf (fd, "%g\n")
		            call pargr (Memr[data+((ii-1)*npix+i)-1])
        	    case TY_DOUBLE:
		        call fprintf (fd, "%g\n")
		            call pargd (Memd[data+((ii-1)*npix+i)-1])
	            }
		}
	    }
	} else {
...
size_t	npix                            #i no. of pixels in each array
int	dtype                           #i type of pointer to alloc
pointer pix                             #o output pixel array ptr
size_t	totpix                          #o total no. of output pixels
...
begin
	...
	if (dtype != TY_CHAR && dtype != TY_UBYTE)
		...
		call achtbs (Mems[ptrs[j]], Mems[ptrs[j]], npix)
	    }
	}
...
begin
	...
	swap = EX_BSWAP(ex)
		...
		call bswap4 (Mems[out], c_1, Mems[out], c_1, O_LEN(op))
		...
		call bswap4 (Memi[out], c_1, Memi[out], c_1, O_LEN(op))
	    ...
	    if ( SZ_LONG == 2 ) {
		if (bitset (swap, S_I4) || bitset (swap, S_ALL)) {
		    call bswap4 (Meml[out], c_1, Meml[out], c_1, O_LEN(op))
		}
	    } else {
		if (bitset (swap, S_I8) || bitset (swap, S_ALL)) {
		    call bswap8 (Meml[out], c_1, Meml[out], c_1, O_LEN(op))
		}
	    }
