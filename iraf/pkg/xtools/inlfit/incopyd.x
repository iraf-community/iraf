...
#	    call pargi (indst)
	...
	if (indst == NULL) {
	    ...
	    call malloc (IN_PARAM  (indst), sz_val, TY_DOUBLE)
	    call malloc (IN_DPARAM (indst), sz_val, TY_DOUBLE)
	    ...
	    sz_val = LEN_INLFLOAT
	    call malloc (IN_SFLOAT (indst), sz_val, TY_DOUBLE)
	...
	call in_bfinitd (indst, in_getl (insrc, INLNPTS),
	...
	call amovd (Memd[in_getp (insrc, INLXMIN)],
		     Memd[in_getp (indst, INLXMIN)], sz_val)
	call amovd (Memd[in_getp (insrc, INLXMAX)],
		     Memd[in_getp (indst, INLXMAX)], sz_val)
end
