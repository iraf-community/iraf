...
begin
	...
	if (IN_PARAM (in) != NULL) {
		...
		call fprintf (fd, "%d -> %g\n")
		    call pargl (j)
		    call pargd (Memd [IN_PARAM (in) + j - 1])
	    }
	} else
	...
	if (IN_PARAM (in) != NULL) {
		...
		call fprintf (fd, "%d -> %g\n")
		    call pargl (j)
		    call pargd (Memd [IN_DPARAM (in) + j - 1])
	    }
	} else
	...
	if (IN_PARAM (in) != NULL) {
		...
		call fprintf (fd, "%d -> %d\n")
		    call pargl (j)
		    call pargl (Meml[IN_PLIST (in) + j - 1])
	    }
	} else
