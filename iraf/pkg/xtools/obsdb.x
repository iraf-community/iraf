...
"noao$base/obsdb.dat".  The observatory may be specified as "observatory"
...
If absent the file is "noao$base/obsdb.dat".  The observatory name
...
begin
	...
	if (envfind ("obsdb", Memc[fname], SZ_FNAME) <= 0) {
	    call strcpy ("noao$base/obsdb.dat", Memc[fname], SZ_FNAME)
	    if (verbose == YES) {
		call eprintf ("Using default observatory database: %s\n")
		    call pargstr ("noao$base/obsdb.dat")
	    }
	} else if (verbose == YES) {
	    call eprintf (
