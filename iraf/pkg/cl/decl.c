...
static const char *badopt      = "Invalid %s option for `%s'.";
static const char *illegal_opt = "Illegal option for `%s'.";
static const char *dup_def     = "Duplicate definition of `%s' ignored.\n";
 ...
 */
int getlimits ( const char *pname, int n, int *i1, int *i2 )
{
	struct param *pp;
	char	*pk, *t, *p, *f;
	int 	dim;
 ...
 */
static int maketype ( int type, int list )
{
	register int	p;
 ...
 */
void do_arrayinit ( struct param *pp, int nval, int nindex )
{
	char *block1;
	char *block2;
 ...
 */
void do_scalarinit ( struct param *pp, int inited )
{
	...
	if (pp->p_type & (PT_LIST|PT_FILNAM|PT_PSET)) {
	    ...
	    if (pvaldefined (pp, s)) {
		strncpy (pp->p_val.v_s, s, len-1);
		pp->p_val.v_s[len-1] = '\0';
	    }
	    else
		pp->p_val.v_s[0] = '\0';
 ...
 */
static int scanftype ( struct param *pp, struct operand *o )
{
	int	type;
	char	*s;
 ...
 */
static int c_scanmode ( struct param *pp, struct operand *o )
{
	if (o->o_type != OT_STRING)
	    return (ERR);
 ...
 */
static int scanlen ( struct param *pp, struct operand *o )
{
	if (o->o_type != OT_INT  ||
 ...
 */
static int scanmin ( struct param *pp, struct operand *o )
{
	int 	bastype, otype;
 ...
 */
static int scanenum ( struct param *pp, struct operand *o )
{
 ...
 */
static int scanmax ( struct param *pp, struct operand *o )
{	
	int	otype;
 ...
 */
struct param *initparam ( struct operand *op, int isparam, int type, int list )
{
	struct	param *pp;
	int	slen;
...
int procscript ( FILE *fp )
{
	...
	while (fgets (buf, PF_MAXLIN+1, fp) != NULL) {
	    for (p = buf; *p == ' '  ||  *p == '\t'; p++)
		;
 ...
 */
int skip_to ( FILE *fp, const char *key )
{
	char	*p, buf[PF_MAXLIN+1];
	int 	count, len;
	long	fpos;
 ...
 * defined by the user.
 */
void do_option ( struct param *pp, struct operand *oo, struct operand *o )
{
	char	*opt;
