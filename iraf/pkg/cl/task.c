 ...
 */
struct ltask *cmdsrch ( const char *pkname, const char *ltname )
{
 ...
 *   error.
 */
struct ltask *ltasksrch ( const char *pkname, const char *ltname )
{
	struct	ltask *ltp;
	struct	package *pkp;
 ...
 */
struct ltask *_ltasksrch ( const char *pkname, const char *ltname, 
 ...
 * NULL if not found.
 */
struct package *pacfind ( const char *name )
{
	struct	package *pkp;
 ...
 */
int defpac ( const char *pkname )
{
	switch ((long)pacfind (pkname)) {
	case 0:
	    return (NO);
	case ERR:
	    cl_error (E_UERR, e_pckambig, pkname);
 ...
 */
/* pkp                  : package to be searched	*/
/* name                 : ltask name			*/
/* enable_abbreviations : enable abbrev. in search	*/
struct ltask *ltaskfind ( struct package *pkp, const char *name, 
 ...
 */
int deftask ( const char *task_spec )
{
	char	buf[SZ_LINE];
 ...
 */
struct ltask *addltask ( struct package *pkp, const char *ptname, 
			 const char *ltname, int redef )
{
	register char *cp;
	const char *cptr;
	register struct ltask *ltp;
	char	*ltbase;
	int	flags;
	char	ltbase_buf[SZ_LINE];

	strncpy(ltbase_buf,ltname,SZ_LINE);
	ltbase_buf[SZ_LINE-1] = '\0';
	 ...
	 */
	if (ptname && (cptr = rindex (ptname, '.')) != NULL) {
	    cptr++;
	    if (!strcmp (cptr, "cl"))
		flags |= LT_SCRIPT;
	    else if (!strcmp (cptr, "par"))
 ...
 */
struct ltask *newltask ( struct package *pkp, const char *lname, 
			 const char *pname, struct ltask *oldltp )
{
 ...
 */
struct package *newpac ( const char *name, const char *bin )
{
	register struct package *pkp;
