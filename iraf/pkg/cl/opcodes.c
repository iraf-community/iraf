...
static int binpipe;			/* last pipe binary or text ? */


void o_undefined ( void )
{
...
{
	const char *argname = (const char *) argp;
	char	*pk, *t, *p, *f;
	struct	pfile *pfp;
	struct	param *pp;
...
void o_addassign ( memel *argp )
{
	 ...
	 */
	const char *pname = (const char *) argp;
	char *pk, *t, *p, *f;
	struct param *pp;
...
void o_indirposset ( memel *argp )
{
	...
	if (pfp->pf_flags & PF_FAKE) {
	    ...
	    o = popop();
	    string_len = strlen (o.o_val.v_s);
	    pp = newfakeparam (pfp, (const char *) NULL, pos, OT_STRING, string_len);
	    pushop (&o);
	} else {
	    pp = paramfind (pfp, (const char *) NULL, pos, NO);
	    if (pp == NULL)
...
{
	const char *funcname = (const char *) argp;
	struct operand o;
	int nargs;
...
{
	const char *command = (const char *)argp;
...
void o_posargset ( memel *argp )
{
	...
	if (pfp->pf_flags & PF_FAKE) {
	    ...
	    if ((o.o_type & OT_BASIC) == OT_STRING)
		string_len = strlen (o.o_val.v_s);
	    pp = newfakeparam (pfp, (const char *) NULL, pos, o.o_type, string_len);
	    pushop (&o);
	} else {
	    pp = paramfind (pfp, (const char *) NULL, pos, NO);
	    if (pp == NULL)
...
void o_pushindex ( int *mode )
{
	...
	if (*mode == 0) {	/* Normal array index reference. */
	    ...
	    n_oarr = stack[stk++];
	    if ( N_OPEN_ARR < n_oarr ) {
		cl_error (E_UERR, "o_pushindex: too large n_oarr\n");
	    }
...
void o_gsredir ( memel *argp )
{
	const char	*ip;
	register FILE	*fp;
	const char *streams = (const char *)argp;
	struct	operand o;
	const char *fname;
	int	count;
...
void o_doscanf( void )
{
	 ...
	 */
	if ( 64 < nargs-1 )
	    cl_error (E_UERR, "scanf: too large nargs\n");
	for (i=0;  i < nargs-1;  i++)
	    o_sv[i] = popop();
	...
	strncpy (format, o.o_val.v_s, SZ_LINE);
	format[SZ_LINE-1] = EOS;
...
void o_dofscanf( void )
{
	...
	if ( 64 < nargs-2 )
	    cl_error (E_UERR, "fscanf: too large nargs\n");
	for (i=0;  i < nargs-2;  i++)
	    o_sv[i] = popop();
	...
	strncpy (pname, o.o_val.v_s, SZ_FNAME);
	pname[SZ_FNAME-1] = EOS;
