...
static void dd_f( const char *, const char * );
 ...
 */
int d_instr ( FILE *fp, const char *prefix, int locpc )
{
	register struct codeentry *cep;
	int opcode, extra=0;
...
void d_t( void )
{
	...
	for ( tp=currentask ;
	      (unsigned long)tp < (unsigned long)&stack[STACKSIZ] ;
	      tp=next_task(tp)) {
	    flags = tp->t_flags;
...
void d_f( void )
{
	...
	dd_f ("physical: ", "hconfig$iraf.h");
}

static void dd_f ( const char *msg, const char *fname )
{
	FILE	*fp[128];
	int	fn;
 ...
 */
void d_fmtmsg ( FILE *fp, const char *prefix, const char *message, int width )
{
	const char *ip;
	register char *op, *cp;
	char *maxcp;
	...
	maxcp = lbuf + SZ_COMMAND -1;
	for (ip=message, op=obuf;  *ip;  ) {
	    /* Get next message line. */
	    for ( cp=lbuf, nchars=0 ; cp < maxcp && (*ip) ; ip++, nchars++ ) {
		if (*ip == '\\' && *(ip+1) == '\n') {
		    *cp++ = *ip;
		    if ( cp < maxcp ) *cp++ = 'n';
		    nchars += 2;
