 ...
 *
 *	edit_history_directive (raw_cmd, new_cmd, bufsize)
 *
...
static	struct param *parmlist[G_MAXPARAM];	/* assoc. keyword with param  */
 ...
 */
/* pset : ltaskname or pfilename */
int epset ( const char *pset )
{
	...
	cx = context;
	cx->e_mpfp = NULL;
	strncpy (cx->e_pset, pset, SZ_FNAME+1);
	cx->e_pset[SZ_FNAME] = EOS;
	...
	while (cx >= context) {
	     ...
	     */
	    if (eparam (cx, &update, &cmd, newpset, SZ_FNAME+1) == ERR) {
		for (;  cx >= context;  --cx) {
		...
		strncpy (cx->e_pset, newpset, SZ_FNAME+1);
		cx->e_pset[SZ_FNAME] = EOS;
		break;
 ...
 * on the old pset.
 */
/* cx       : eparam editor context	*/
/* update   : update pset upon exit	*/
/* nextcmd  : receives next command	*/
/* nextpset : receives next pset name	*/
/* bufsize  : buffer size of nextpset	*/
	     ...
	     int bufsize )
{
	...
	while (nextline != cmdline) {
	     ...
	     */
	    e_encode_vstring (parmlist[keyid], string, G_MAXSTRING);
...
exit:
	...
	*update = ep_update;
	*nextcmd = ep_nextcmd;
	strncpy (nextpset, e_nextpset, bufsize);
	nextpset[bufsize - 1] = EOS;
...
int e_makelist ( struct pfile *pfileptr )
{
	...
	for (pp = pfileptr->pf_pp, numkeys = 0;  pp != NULL;  pp = pp->p_np) {
	    ...
	    if ( G_MAXPARAM <= numkeys + 1 ) {
		eprintf ("eparam: too large numkeys\n");
		break;
	    }
	...
	if (cldebug) {
	    ...
	    E_DEBUG (dbg);
	    for (i=1;  i<= maxpage;  i++) {
		snprintf (dbg, SZ_LINE, "topkeys : %d  ", topkeys[i]);
	        E_DEBUG (dbg);
	    }
	    snprintf (dbg, SZ_LINE, "numkeys = %d  ", numkeys);
	    E_DEBUG (dbg);
	}    
 ...
 * start a new page.
 */
/* cur : current line count on screen */
/* new : new count, returned if new page */
int e_testtop ( int cur, int new )
{
	if (cur > (botline - topline + 1) && maxpage + 1 < G_MAXPAGES ) {
	    topkeys[++maxpage] = numkeys;
	    return (new);
	} else {
	    if ( G_MAXPAGES <= maxpage + 1 ) {
		eprintf ("eparam: too large maxpage\n");
	    }
	    return (cur);
	}
}
...
void e_repaint( void )
{
	static const char *static_prompt = "--------- parameter array ---------";
	...
	for (keyid=topkey;  keyid <= botkey;  keyid++) {
		...
		if (parmlist[keyid]->p_mode & M_HIDDEN)
		    snprintf (outbuf, MAXPROMPT, "(%-7.7s) ", parmlist[keyid]->p_name);
		else
		    snprintf (outbuf, MAXPROMPT, "%-8.8s  ", parmlist[keyid]->p_name);
 ...
 */
/* pfp     : pfile pointer			*/
/* cmdline : terminal command line number	*/
/* maxcol  : max cols on a line			*/
void e_pheader ( struct pfile *pfp, int cmdline, int maxcol )
{
	static const char *logo = "  I R A F  ";
	static const char *title= "Image Reduction and Analysis Facility";
	...
	e_ctrl ("ue");
	if (ep_filemode) {
	    snprintf (string, SZ_LINE+1, "PARFILE = %s\r\n", pfp->pf_pfilename);
	    e_putline (string);
	} else {
	    struct  ltask *ltp = pfp->pf_ltp;
	    snprintf (string, SZ_LINE+1, "PACKAGE = %s\r\n", ltp->lt_pkp->pk_name);
	    e_putline (string);
	    snprintf (string, SZ_LINE+1, "   TASK = %s\r\n", ltp->lt_lname);
	    e_putline (string);
	}

	for ( col=0 ; col < maxcol && col < SZ_LINE+1-1  ; col++ )
	    string[col] = ' ';
        string[col] = '\0';
	e_ctrl ("us");
...
void e_drawkey( void )
{
	...
	e_encode_vstring (parmlist[keyid], valuebuf, MAXPROMPT);
	e_goto (1, line);
	e_clrline();
	...
	if (parmlist[keyid]->p_type & PT_ARRAY) {
	     ...
	     */
	    if (hidden)
		snprintf (tempbuf, MAXPROMPT, "(%-7.7s=", parmlist[keyid]->p_name);
	    else
	        snprintf (tempbuf, MAXPROMPT, "%-8.8s=", parmlist[keyid]->p_name);
	    e_putline (tempbuf);
		...
		if ( strlen(valuebuf) + 1 < MAXPROMPT ) strcat (valuebuf, ")");
	    }
	    e_putline (valuebuf);
 ...
 * spaces to indent.
void e_indent_prompt ( char *p, char *bp, int indent )
{
	register int	i;
	register char	c;
 ...
 */
void e_encode_vstring ( struct param *pp, char *outbuf, int bufsize )
{
	char valuebuf[G_MAXSTRING];
	char colbuf[16];

	if ( 0 < bufsize ) *outbuf = '\0';
	...
	if (pp->p_type & PT_ARRAY) {
		...
		if (opundef (&o))
			 ...
			 */
			snprintf (colbuf, 16-1, "%10g ", o.o_val.v_r);
		        if (index (colbuf, '.') == NULL) {
			    strcat (colbuf, ".");	/* ??? */
			}
		    } else {
			poffset (i-1);
		        spparval (valuebuf, G_MAXSTRING, pp);
		    	snprintf (colbuf, 16, "%10.10s ", valuebuf);
		    }
		}

		strncat(outbuf,colbuf,
		(strlen(outbuf) < (bufsize)) ? (bufsize)-1-strlen(outbuf) : 0);
	    }
		...
		if ( 0 < bufsize ) *outbuf = EOS;
	    }
	    else {
		sprop (outbuf, bufsize, &pp->p_valo);
	    }
	}
}
 ...
 */
void e_check_vals ( const char *string )
{
	const char *errstr;	/* pointer to the error string (or 0)     */
	...
	if (cldebug) {
	    snprintf (dbg, SZ_LINE, "string = |%s|  ", string);
	    E_DEBUG (dbg);
	}
	...
	if (isarray) {
		...
		if (strcmp (errstr, "OK") != 0) { 
		    snprintf (message, SZ_LINE+1, "%s [%s]?", errstr, outstring);
		    badnews++;
		}
	    }

	    if ((nelem != numonrow) && !(badnews)) {
		snprintf (message, SZ_LINE+1, "Expected %d elements on this line",numonrow);
		badnews++;
	    }
	    ...
	    if (strcmp (errstr, "OK") != 0) {
		strncpy (message, errstr, SZ_LINE+1);
		message[SZ_LINE] = EOS;
		badnews++;
	    }
	}
 ...
 * after the token.
 */
/* ip      : pointer into input string	*/
/* outstr  : receives token		*/
/* bufsize : buffer size of outstr	*/
char *e_getfield ( const char *ip, char *outstr, int bufsize )
{
	register char	*op, *otop;

	while (*ip == ' ' || *ip == ',')
	    ip++;
	otop = outstr + bufsize -1;
	op = outstr;
	while ( op < otop && *ip != '\0' && *ip != ' ' && *ip != ',' ) 
	    *op++ = *ip++;
	if ( op <= otop ) *op = '\0';
 ...
 */
/* args : ehistory argument list			*/
/* new_cmd : the command to be executed after editing	*/
int edit_history_directive ( const char *args, char *new_cmd, int bufsize )
{
	 ...
	 */
	arglist[0] = '^';
	strncpy (&arglist[1], args, SZ_LINE);
	arglist[SZ_LINE] = EOS;
	execute = process_history_directive (arglist, new_cmd, bufsize);
	...
	while (ice) {
	    ...
	    *(new_cmd+numchar+1) = '\0';
		...
		if (get_history (record, new_cmd, bufsize) == ERR)
		    ice = NO;
	    } else if (nextline > botline) {
		record = what_record() - 1;
		if (get_history (record, new_cmd, bufsize) == ERR)
 ...
 */
/* eparam : flag to indicate eparam or ehis  */
int editstring ( char *string, int eparam )
{
	...
	if (eparam) {
	     ...
	     */
	    strncpy (oldline, string, G_MAXSTRING);
	    oldline[G_MAXSTRING - 1] = EOS;
	    numchar = 0;
	    cp  = string;
	    *cp = '\0';
	} else {
...
void e_ttyinit( void )
{
	 ...
	 */
	e_ucasein  = c_ttstati (STDIN,  TT_UCASEIN);
	e_ucaseout = c_ttstati (STDOUT, TT_UCASEOUT);
...
int e_colon( void )
{
	...
	ucasein_set = c_ttstati (STDIN, TT_UCASEIN);
...
again_:
	...
	*op = EOS;
	...
	case 'w':
	    ...
	    snprintf (buf, SZ_LINE+1, " - %d parameters written to %s", n,
	...
	case 'e':
		 ...
		 */
		if (e_psetok (pset)) {
		    strncpy (e_nextpset, pset, SZ_FNAME+1);
		    e_nextpset[SZ_FNAME] = EOS;
		    ep_nextcmd = EP_EDIT;
		    return (EP_EOF);
		} else
		    return (ERR);
		...
		if (!(pp->p_type & PT_PSET)) {
		    snprintf (buf, SZ_LINE+1, 
			      "parameter `%s' is not a pset parameter",
			      pp->p_name);
		    e_puterr (buf);
		    return (ERR);
		}
		 ...
		 */
		e_encode_vstring (pp, buf, SZ_LINE+1);
		if (*buf == EOS)
		    pset = pp->p_name;
		else
		    pset = buf;
...
error_:
	snprintf (errmsg, SZ_LINE+1, errfmt, errarg);
	e_puterr (errmsg);
	return (NO);
}
 ...
 */
void e_puterr ( const char *errmsg )
{
	c_ttygoto (tty_fd, tty, 1, cmdline);
	c_ttyclearln (tty_fd, tty);
	e_putline (errmsg);
...
int e_movedown ( int eparam )
{
	...
	if (cldebug) {
	    snprintf (dbg, SZ_LINE, "nextline=%d, nextkey=%d line=%d keys=%d", 
 ...
 */
char *e_tonextword ( const char *ip )
{
	ip++;
 ...
 */
char *e_toprevword ( const char *ip, const char *string )
{
	--ip;
 ...
 */
void e_putline ( const char *stwing )
{
	const char	*ip;
	register char	*op;
	register int	ch;
	 ...
	 */
	if (e_ucaseout) {
	    char *maxop = obuf + 512 -1;
	    for ( ip=stwing, op=obuf ; (ch = *ip) != EOS && op < maxop ; ) {
...
/* sline, scol : starting line and column	*/
void e_display ( const char *string, int sline, int scol )
{
...
/* string      : string to be printed			*/
/* sline, scol : starting line and column		*/
/* ccol        : start col of continuation lines	*/
void e_displayml ( const char *string, int sline, int scol, int ccol )
{
	const char	*ip;
	register char	*op;
	char *maxop;
	...
	for (ip=string, op=lbuf;  *ip != EOS;  ) {
	    for ( op=lbuf ; (*ip) != EOS && op < maxop ; op++, ip++ ) {
		*op = *ip;
		if (*op == '\n') {
		    *op = EOS;
