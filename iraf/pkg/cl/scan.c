...
static void lentst ( char * );

void cl_scan ( int nargs, const char *source )
{
	char	buf[SZ_LINE];
	char	*bp, *start, c;
	char	*pk, *t, *p, *f;
 ...
 */
void cl_scanf ( const char *format, int nargs, const char *input )
{
	...
	if ( MAXARGS < nargs ) {
	    cl_error (E_UERR, "cl_scanf: too large nargs\n");
	}
