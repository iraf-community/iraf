...
static const char *uparmdir = UPARM;

static long filetime ( const char *, const char * );
static void mkpfilename ( char *, const char *, const char *, const char *, const char * );
static int scantype ( const char * );
static int ck_atoi ( const char *, int * );
static void mapname ( const char *, char *, int );
 ...
 */
/* pfp : pfile to be unlinked	*/
void pfileunlink ( struct pfile *pfp )
{
 ...
 */
/* ltp : ltask descriptor */
struct pfile *pfilefind ( struct ltask *ltp )
{
 ...
 * descriptor.
 */
/* pfilepath : filename or ltask pathname	*/
struct pfile *pfilesrch ( const char *pfilepath )
{
	struct	pfile *pfp;
	...
	if (is_pfilename (pfilepath)) {
	    ...
	    strncpy (pfp->pf_pfilename, pfilepath, SZ_FNAME+1);
	    pfp->pf_pfilename[SZ_FNAME] = EOS;
	    return (pfp);
 ...
 */
/* ltp : ltask descriptor */
struct pfile *pfileload ( struct ltask *ltp )
{
...
epset_:
	...
	c_fnldir (ltp->lt_pname, pkgdir, SZ_FNAME+1);
	 ...
	 * This is indicated by the modify time of the special file hconfig$utime,
	 ...
	 */
	if (sys_ftime <= 0)
	    sys_ftime = filetime ("hconfig$utime", "m");
 ...
 */
/* npf    : loaded parameter set	*/
/* opfile : old parameter file		*/
int pfilemerge ( struct pfile *npf, const char *opfile )
{
	...
	for (n_pp = NULL, o_pp = opf->pf_pp;  o_pp;  o_pp = o_pp->p_np) {
		...
		if (((o_pp->p_valo.o_type & OT_BASIC) == OT_STRING) &&
		   ...
		   n_pp->p_val.v_s[n_pp->p_lenval-1] = EOS;
		} else
 ...
 */
/* ltp       : associated ltask			*/
/* pfilename : parameter file filename		*/
/* checkmode : check for "mode" parameter	*/
struct pfile *pfileread ( struct ltask *ltp, const char *pfilename, 
			  int checkmode )
{
	const char *ip;
	char	buf[PF_MAXLIN];
	...
	save_topd = topd;
	pfp = newpfile (ltp);
	strncpy (pfp->pf_pfilename, pfilename, SZ_FNAME+1);
	pfp->pf_pfilename[SZ_FNAME] = EOS;
 ...
 * always ".par".
 */
/* pfp       : pfile descriptor		*/
/* pfilename : file to be written	*/
int pfilewrite ( struct pfile *pfp, const char *pfilename )
{
	const char *ip;
	char	*op, *maxop, *dot;
	 ...
	 */
	maxop = pfname + SZ_PATHNAME+1 -1;
	dot=NULL;
	ip=pfilename;
	for ( op=pfname ; op < maxop && (*ip) ; op++, ip++ ) {
	    *op = *ip;
	    if (*op == '.') dot = op;
	}
	if ( dot == NULL ) dot = op;
	if ( dot < maxop - 4 )
	    ...
	    cl_error (E_IERR, "in pfilewrite: too long pfilename");
...
int pfileinit ( struct ltask *ltp )
{
	struct	task *tp;
	struct	pfile *pfp;
	char	pfilename[SZ_FNAME+1];	/* user pfile			*/
	char	pkgdir[SZ_FNAME+1];
	const char *ltname;		/* name of the new pfile	*/
	const char *pkname;		/* name of its package		*/
	int	running;
 ...
 */
int is_pfilename ( const char *opstr )
{
	const char *ip;
	const char *dot;
 ...
 * Size of buf : SZ_FNAME + 1
 */
/* buf    : receives output filename	*/
/* dir    : dir name or prefix		*/
/* pkname : package name		*/
/* ltname : ltask name			*/
/* extn   : filename extension		*/
static void mkpfilename ( char *buf, const char *dir, const char *pkname, 
			  const char *ltname, const char *extn )
{
	char	temp[SZ_FNAME+1];

	strncpy (buf, dir, SZ_FNAME);	/* start with directory name	*/
	buf[SZ_FNAME] = EOS;

	if (strcmp (dir, uparmdir) == 0) {
	    if ( strlen(buf) + 1 < SZ_FNAME + 1 ) strcat(buf, "$");
	    mapname (pkname, temp, LEN_PKPREFIX);
	    if ( strlen(buf) + strlen(temp) < SZ_FNAME + 1 ) strcat(buf, temp);
	    mapname (ltname, temp, LEN_PFILENAME);
	    if ( strlen(buf) + strlen(temp) < SZ_FNAME + 1 ) strcat(buf, temp);
	} else {
	    if ( strlen(buf) + strlen(ltname) < SZ_FNAME + 1 ) strcat(buf, ltname);
	}

	if ( strlen(buf) + strlen(extn) < SZ_FNAME + 1 )
 ...
 */
static void mapname ( const char *in, char *out, int maxlen )
{
	register int ip, op;
	...
	while (op < maxlen-1 && (out[op++] = in[ip++]) != '\0')
	    ;
	if ( 0 < op && out[op-1] != '\0' ) {	/* append last char	*/
	    if (op < maxlen) {
		if (in[ip] != '\0') {
		    while (in[ip] != '\0')
			ip++;
...
/* fname    : file name		*/
/* timecode : "c" or "m"	*/
static long filetime ( const char *fname, const char *timecode )
{
	struct	_finfo fi;
 ...
 */
struct pfile *pfilecopy ( struct pfile *pfp )
{
	...
	for (pp = pfp->pf_pp;  pp;  pp = pp->p_np) {
		...
		newpp->p_val.v_s[pp->p_lenval-1] = EOS;
	    }
		...
		newpp->p_min.v_s[PF_SZMINSTR-1] = EOS;
	    }
		...
		newpp->p_max.v_s[PF_SZMAXSTR-1] = EOS;
	    }
	...
	for (pt=pft->pf_pp, pf=pff->pf_pp;  pf&&pt;  pt=pt->p_np, pf=pf->p_np) {
		...
		if (((pf->p_valo.o_type & OT_BASIC) == OT_STRING) &&
		   (pt->p_val.v_s != NULL)) {
		    ...
		    pt->p_val.v_s[pf->p_lenval-1] = EOS;
		} else
		...
		if ((pf->p_mino.o_type & OT_BASIC) == OT_STRING &&
		    ...
		    pt->p_min.v_s[PF_SZMINSTR-1] = EOS;
		} else
		...
		if ((pf->p_maxo.o_type & OT_BASIC) == OT_STRING &&
		    ...
		    pt->p_max.v_s[PF_SZMAXSTR-1] = EOS;
		} else
 ...
 */
struct param *addparam ( struct pfile *pfp, const char *buf, FILE *fp )
{
	static const char *minfields =
		"must specify at least name,type,mode for `%s'\n";
	static const char *nominmax =
		"ranges not allowed for struct/cursor/string/bool param `%s'\n";
	static const char *umquotes =
	...
	const char *pnamehold;	/* param's name as soon as we know it	*/
 ...
 */
static int ck_atoi ( const char *str, int *val )
{
	const char *s;
 ...
 */
char *nextfield ( const char **pp, FILE *fp )
{
	static char readbuf[SZ_LINE];
	register char c;	/* fast references to field	 	*/
	const	char *p;
	...
	if (c == '\'' || c == '"') {
		...
		if (c == '\n') {
			...
			continue;
		    case 'n':
			if ( bp < buf + SZ_LINE ) *bp++ = '\n';
			break;
		    case 't':
			if ( bp < buf + SZ_LINE ) *bp++ = '\t';
			break;
		    case 'r':
			if ( bp < buf + SZ_LINE ) *bp++ = '\r';
			break;
		    case 'f':
			if ( bp < buf + SZ_LINE ) *bp++ = '\f';
			break;
		    case '\'':
		    case '"':
			if ( bp < buf + SZ_LINE ) *bp++ = c;
			break;
		    default:
			if ( bp < buf + SZ_LINE ) *bp++ = '\\';		/* preserve esc seq.	*/
			if ( bp < buf + SZ_LINE ) *bp++ = c;
			break;
		    }
		} else if (c == '\0' || c == quote) {
		    if ( buf + SZ_LINE -1 < bp ) {
			cl_error (E_IERR, "in nextfield: bp exceeds size of buf");
		    }
		    *bp = '\0';
		    if (start == NULL)
			start = comdstr (buf);
 ...
 */
int scanmode ( const char *s )
{
	register int mode = 0;
	const char *str, *ip;
	char *op, *maxop;
	static const char *badstr = "bad mode string `%s'";
	char	strings[4][25];
	int	i, n;
	...
	if (index (str, ',') != NULL || index (str, '+') != NULL) {
		...
		while (*ip == ' ' || *ip == '\t')
		    ip++;
		maxop = strings[n] + 25 -1;
		for ( op=strings[n] ; op < maxop && *ip != '\0' ; op++, ip++ ) {
		    if (!isalpha (*ip)) break;
		    *op = *ip;
		}
		*op = '\0';
 ...
 */
static int scantype ( const char *s )
{
	static const char *badtype =  "bad type spec `%c'";
	static const char *cnfltype = "conflicting type spec `%c'";
	char sbuf[16];
	register int type;
	...
	} else if (!strcmp (makelower (sbuf), "struct")) {
	    type |= (PT_STRUCT|OT_STRING);
	} else if (!strcmp (makelower (sbuf), "gcur")) {
	    type |= (PT_GCUR|OT_STRING);
	} else if (!strcmp (makelower (sbuf), "imcur")) {
	    type |= (PT_IMCUR|OT_STRING);
	} else if (!strcmp (makelower (sbuf), "ukey")) {
	    type |= (PT_UKEY|OT_STRING);
	} else if (!strcmp (makelower (sbuf), "pset")) {
