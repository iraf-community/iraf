...
static int mode_offset = INDEX_OFFSET;

static const char *loc_field = "Attempt to access undefined field in local variable %s.\n";

static void qputs ( const char *, FILE * );
 ...
 */
struct param *paramfind ( struct pfile *pfp, const char *pname, int pos, 
			  int exact)
{
 ...
 */
void paramset ( struct param *pp, char field )
{
	struct	operand o;
	...
	case FN_MIN:				 /* minimum */
		...
		if (opindef (&o))
		    ...
		    pp->p_min.v_s[PF_SZMINSTR-1] = EOS;
	...
	case FN_MAX:				 /* maximum */
		...
		if (opindef (&o))
		    ...
		    pp->p_max.v_s[PF_SZMAXSTR-1] = EOS;
 ...
 */
void validparamget ( struct param *pp, char field )
{
	struct operand o;
 ...
 */
void makemode ( struct param *pp, char *s, int bufsize )
{
	register int m = pp->p_mode;

	if ( bufsize < 5 ) 
	    cl_error (E_IERR,"makemode: too small bufsize");
	if (m & M_AUTO)
	    *s++ = PF_AUTO;
	if (m & M_QUERY)
	    *s++ = PF_QUERY;
 ...
 */
struct param *paramsrch ( const char *pkname, const char *ltname, 
			  const char *pname )
{
	register struct param *pp;
	struct	pfile *pfp;
	...
	if (currentask->t_pfp->pf_flags & PF_FAKE) {
		...
		pp = paramfind (pfp, (const char *)NULL, pfp->pf_n++, NO);
 ...
 */
int defpar ( const char *param_spec )
{
	char	sbuf[SZ_LINE];
	char	*pkname, *ltname, *pname, *junk;

	strncpy (sbuf, param_spec, SZ_LINE);
	sbuf[SZ_LINE-1] = EOS;
	breakout (sbuf, &pkname, &ltname, &pname, &junk);

	switch ((long) lookup_param (pkname, ltname, pname)) {
	case 0:
	case ERR:
	    return (NO);
	default:
 ...
 */
struct param *lookup_param ( const char *pkname, const char *ltname, 
			     const char *pname )
{
	register struct param *pp;
 ...
 */
int printparam ( struct param *pp, FILE *fp )
{
 ...
 */
static void qputs ( const char *str, FILE *fp )
{
	register char	ch;
 ...
 * value.
 */
int pvaldefined ( struct param *pp, const char *s )
{
	int val;
 ...
 */
/* string_len : if new param is type string, size of string */
struct param *newfakeparam ( struct pfile *pfp, const char *name, int pos, 
