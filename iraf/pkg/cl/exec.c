...
static void set_clio ( struct task * );
static void mk_startupmsg ( struct task *, char *, int );
 ...
 */
void callnewtask ( const char *name )
{
...
void execnewtask ( void )
{
	...
	if (pp == NULL || (long)pp == ERR) {
	    char nabuf[FAKEPARAMLEN];
	    snprintf (nabuf, FAKEPARAMLEN, "$nargs,i,h,%d\n", pfp->pf_n);
	...
	if (newtask->t_flags & T_PSET) {
	     ...
	     */
	    mk_startupmsg (newtask, startup_msg, SZ_STARTUPMSG+1);
	...
	if (keeplog() && log_trace())
	    if (newtask->t_flags & T_SCRIPT || newtask->t_pid != -1) {
	    	char  logmsg[SZ_LINE];
	    	snprintf (logmsg, SZ_LINE, 
			  "Start (%s)", newtask->t_ltp->lt_pname);
 ...
 */
/* tp      : task being executed		*/
/* cmd     : receives formatted command		*/
/* bufsize : buffer size of cmd			*/
static void mk_startupmsg ( struct task *tp, char *cmd, int bufsize )
{
	register char	*ip, *op, *maxop, *cp, *maxcp;
	struct	pfile *pfp;
	struct	operand o;
	struct	param *pp;
	char	redir[20];
	...
	while (*ip == CH_INVIS)
	    ip++;
	strncpy (cmd, ip, bufsize);
	if ( 0 < bufsize ) cmd[bufsize-1] = EOS;
	...
	if (tp->t_flags & (T_MYIN|T_MYOUT|T_MYERR)) {
		...
		strncat(cmd," < $",
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    if (tp->t_flags & T_MYOUT)
		strncat(cmd," > $",
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    if (tp->t_flags & T_MYERR) {
		snprintf (redir, 20, " %d> $", STDERR);
		strncat(cmd,redir,
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    }
	}
	if (tp->t_flags & (T_MYSTDGRAPH|T_MYSTDIMAGE|T_MYSTDPLOT)) {
		...
		snprintf (redir, 20, " %d> $", STDGRAPH);
		strncat(cmd,redir,
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    }
	    if (tp->t_flags & T_MYSTDIMAGE) {
		snprintf (redir, 20, " %d> $", STDIMAGE);
		strncat(cmd,redir,
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    }
	    if (tp->t_flags & T_MYSTDPLOT) {
		snprintf (redir, 20, " %d> $", STDPLOT);
		strncat(cmd,redir,
			(strlen(cmd) < bufsize) ? bufsize-1-strlen(cmd) : 0);
	    }
	}
	...
	for (pfp = tp->t_pfp;  pfp;  pfp = pfp->pf_npset) {
		...
		if (!(pp->p_type & PT_LIST) && !(effmode(pp) & M_QUERY)) {
		     ...
		     * pset-param pfile.
		     */
		    maxop = buf + SZ_LINE+1 -1;
		    if (pfp->pf_psetp != NULL) {
			ip = pfp->pf_psetp->p_name;
			for ( op=buf ; op < maxop && (*ip) ; op++, ip++ )
			    *op = *ip;
			if ( op < maxop ) *op++ = '.';
		    } else
			op = buf;

		    for ( ip=pp->p_name ; op < maxop && (*ip) ; op++, ip++ )
			*op = *ip;
		    if ( op < maxop ) *op++ = '=';
		     ...
		     */
		    sprop (val, SZ_LINE+1, &pp->p_valo);
		    if ((pp->p_type & OT_BASIC) == OT_STRING) {
			if ( op < maxop ) *op++ = '"';
		    }

		    for ( ip=val ; op < maxop && (*ip) ; ip++ ) {
			if (*ip == '\n') {
			    *op = '\\';
			    if ( op < maxop ) *op++;
			    *op = 'n';
		    ...
		    *op = EOS;
		     ...
		     */
		    if ( maxcp < cp + 2 + strlen(buf) + 1 ) break;
 ...
 */
/* pkg      : package in which task resides			*/
/* pkg_path : pathname of exe file given in TASK statement	*/	
char *findexe ( struct package *pkg, const char *pkg_path )
{
	static	char bin_path[SZ_PATHNAME+1];
	char	root[SZ_FNAME+1];

	c_fnroot (pkg_path, root, SZ_FNAME+1);
	snprintf (bin_path, SZ_PATHNAME+1, 
		  "%s%s.e", pkg ? pkg->pk_bin : BINDIR, root);

	if (c_access (bin_path, 0, 0) == YES)
	    return (bin_path);
	else
	    return ((char *)pkg_path);
}
 ...
 */
static void set_clio ( struct task *newtask )
{
 ...
 */
/* pfp    : first pfile in chain	*/
/* tn     : psetname (taskname) or null	*/
/* pn     : parameter name		*/
/* pos    : for paramfind		*/
/* abbrev : for paramfind		*/
struct param *ppfind ( struct pfile *pfp, const char *tn, const char *pn, 
		       int pos, int abbrev )
{
 ...
 */
static void printcall ( FILE *fp, struct task *tp )
{
	register struct param *pp;
	int	notfirst = 0;
