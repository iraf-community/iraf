...
static const char *nextstr ( const char **, FILE * );
static const char *nxtchr ( const char *, FILE * );
static void pquery ( struct param *, FILE * );
static char *bkg_query ( char *, int, struct param * );
...
void query ( struct param *pp )
{
	...
	forever {
		 ...
		 */
		query_status = bkg_query (buf, SZ_PROMPTBUF+1, pp);
		...
		if (pp->p_type & PT_GCUR) {
		    if (c_envfind ("stdgcur", source, 33) <= 0)
			strcpy (source, "stdgraph");
		} else {
		    if (c_envfind ("stdimcur", source, 33) <= 0)
		...
		if (cursor == STDIMAGE) {
			...
			&x,&y,&wcs,&key,str,SZ_LINE+1, 1, 1) == EOF) {
			query_status = NULL;

		    } else {
			if (isprint(key) && !isspace(key))
			    snprintf (keystr, 10, "%c", key);
			else
			    snprintf (keystr, 10, "\\%03o", key);
			snprintf (buf, SZ_PROMPTBUF+1, "%.3f %.3f %d %s %s\n",
			    x, y, wcs, keystr, str);
			query_status = (char *)strlen(buf);
		    }

		} else if (c_rcursor (cursor, buf, SZ_PROMPTBUF+1) == EOF) {
		    query_status = NULL;
		} else
		    query_status = (char *)strlen(buf);
		 ...
		 */
		pp->p_flags &= ~P_LEOF;
		if (c_rdukey (buf, SZ_PROMPTBUF+1) == EOF)
		    query_status = NULL;
		else
		    query_status = (char *)strlen(buf);

	    } else {
text_query:	fd = spf_open (buf, SZ_PROMPTBUF+1);
		pquery (pp, fdopen(fd,"a"));
		spf_close (fd);

		c_stgputline (STDOUT, buf);
		if (c_stggetline (STDIN, buf, SZ_PROMPTBUF+1) > 0)
		    query_status = (char *) strlen(buf);
		else
		    query_status = NULL;
	    }
		...
		*nlp = '\0';			/* cancel the newline	*/
		...
		if (arrflag) {
		    ...
		    if (ip == NULL  ||  ip == (const char *) ERR  ||  ip == undefval)
			goto testval;
		}
...
testval:
	...
	if (arrflag  &&  query_status != NULL  &&  *ip != '\0') {
		...
		if (ip == (const char *) ERR) {	/* Error on query line. */
 ...
 */
static const char *nextstr ( const char **pbuf, FILE *fp )
{
	const char *p;
 ...
 */
static const char *nxtchr ( const char *p, FILE *fp )
{
 ...
 */
static void pquery ( struct param *pp, FILE *fp )
{
	struct	operand o;
	int	offset, arrflag;
 ...
 */
/* obuf : same calling sequence as 'fgets' */
static char *bkg_query ( char *obuf, int bufsize, struct param *pp )
{
	...
	get_bkgqfiles (bkgno, ppid, bqfile, SZ_PATHNAME, qrfile, SZ_PATHNAME);
	...
	fgets_status = fgets (obuf, bufsize, in);
	fclose (in);
	c_delete (qrfile);
 ...
 */
/* bkgno : ordinal of job requiring service	*/
void service_bkgquery ( int bkgno )
{
	...
	if (bkg_jobactive (bkgno) == NO)
	    ...
	    get_bkgqfiles (bkgno, c_getpid(), 
			   bqfile, SZ_PATHNAME, qrfile, SZ_PATHNAME);
	 ...
	 */
	c_delete (qrtemp);
	fgets (response, SZ_LINE+1, stdin);
 ...
 */
void get_bkgqfiles ( int bkgno, int pid, 
		     char *bkg_query_file, int bufsize_q, 
		     char *query_response_file, int bufsize_r )
{
	int	filecode;
	...
	snprintf (bkg_query_file, bufsize_q, 
		  "%sBQF%d", envget(UPARM), filecode);
	snprintf (query_response_file, bufsize_r, 
		  "%sBQR%d", envget(UPARM), filecode);
}
 ...
 */
int inrange ( struct param *pp, struct operand *op )
{
	...
	if (fulltype & PT_FILNAM) {
	     ...
	     */
	    const char *filnam = op->o_val.v_s;
	    if (opindef (op))
		return (NO);
