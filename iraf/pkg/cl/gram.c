...
char truestr[4] = {'y','e','s','\0'};	/* true constant as it appears in ASCII	*/
char falsestr[3] = {'n','o','\0'};	/* false                "		*/
char nullstr[1] = { EOS };
char undefval[1] = { EOS };	/* used in nextfield(), pvaldefined()	*/

const char *indefstr = "INDEF";	/* input or output for indef operands	*/
const char *indeflc = "indef";	/*   lower case version.		*/
const char *eofstr = "EOF";	/* list file EOF or input		*/
const char *eoflc = "eof";	/*   lower case version			*/
const char *epsilonstr="epsilon";	/* a small value; see config.h	*/
const char *errorstr = "error";	/* the error statement			*/

char *err_cmdblk;		/* Pointer where error detected		*/

static void pretty_param ( struct param *, FILE * );
static void show_param ( struct ltask *, struct param *, FILE * );
static int fieldcvt ( char * );
static int keyword ( const char *[], const char * );
static char *pipefile ( int );
 ...
 */
int crackident ( const char *s )
{
	struct keywords {
		const char *k_name;	/* the keyword string itself.	*/
	...
	if (sch == *truestr && !strcmp (scopy, truestr)) {
	    ...
	    char sb[REALWIDTH];
	    snprintf (sb, REALWIDTH, "%e", EPSILON);
 ...
 */
int addconst ( const char *s, int t )
{
 ...
 */
static void pretty_param ( struct param *pp, FILE *fp )
{
	register char	ch, *p;
	char	buf[SZ_LINE];
	int	nchars, maxch;
	...
	if (pp->p_mode & M_HIDDEN) {
	    snprintf (buf, SZ_LINE, "(%0.12s = ", pp->p_name);
	}
	else {
	    snprintf (buf, SZ_LINE, "%0.12s = ", pp->p_name);
	}
	...
	if (pp->p_type & PT_ARRAY) {
		...
		if (amin != 1)
		    snprintf (ibuf, 15, "%d:%d", amin, amax);
		else
		    snprintf (ibuf, 15, "%d", amax);
	
		strncat(buf, ibuf, SZ_LINE-1-strlen(buf));
		if (d+1<dim)
		    strncat(buf, ",", SZ_LINE-1-strlen(buf));
 ...
 */
static void show_param ( struct ltask *ltp, struct param *pp, FILE *fp )
{
	char	buf[SZ_LINE+1];
	int	isstr;
...
void listhelp ( struct package *pkp, int show_invis )
{
	...
	for (ltp = pkp->pk_ltp;  ltp != NULL;  ltp = ltp->lt_nlt) {
	    ...
	    /* Get task name. */
	    list[nltask++] = op;
	    for ( ip=ltp->lt_lname ; op < maxop && (*ip) ; op++, ip++ )
		*op = *ip;
 ...
 *   e.g. = task.array[*]
 */
void breakout ( const char *full, char **pk, char **t, char **p, char **f )
{
	register char *cp;
	register int npts, n;
	char	*pts[3];
 ...
 */
static int fieldcvt ( char *f )
{
	/* Field name and corresponding code tables.
	 */
	static const char *fntbl[] = {
 ...
 * at which s matched.
 */
static int keyword ( const char *tbl[], const char *s )
{
	const char *kentry;
	int cand, len, i;
 ...
 */
void intrfunc ( const char *fname, int nargs )
{
 ...
 */
struct operand sexa ( const char *s )
{
	struct operand o;
	int	n, sign;
	int	hr, minutes;
 ...
 */
static char *pipefile ( int pipecode )
{
	...
	if (envget ("pipes") != NULL)
	    ...
	    dir = "tmp$";
	snprintf (fname, SZ_PIPEFILENAME+1, "%spipe%d", dir, pipecode);
...
{
	struct	operand	*o;
	static	const char *badcase = "Invalid case constant.";
	int	ival;
