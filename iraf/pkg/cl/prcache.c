 ...
 *   cl$cl.par or in hconfig$clpackage.par, hence may vary from site to site
...
static int pr_pno = 1;			/* incremented for each connect	*/
static int sz_prcache = 2;		/* nprocess slots in cache	*/
static struct process pr_cache[MAXSUBPROC];
static struct process *pr_head = NULL, *pr_tail = NULL;
 ...
 * efficiency of pipes.
 */
/* process            : filename of process		*/
/* command            : IRAF Main command		*/
/* in, out            : IPC channels (output)		*/
/* t_in, t_out, t_err :  task stdin,out,err (input)	*/
/* t_gr, t_im, t_pl   : task graphics streams		*/
/* timeit             : if !0, time command		*/
int pr_connect ( const char *process, const char *command, 
		 FILE **in, FILE **out, FILE *t_in, FILE *t_out, FILE *t_err, 
		 FILE *t_gr, FILE *t_im, FILE *t_pl, int timeit )
{
	register int	pid;
 ...
 */
/* pid : process id returned by connect	*/
...
/* process : filename of process	*/
/* in, out : IPC channels (output)	*/
int pr_pconnect ( const char *process, FILE **in, FILE **out )
{
	register struct process *pr;
	struct	_finfo fi;
	int	fd_in, fd_out;
	...
	if (pr != NULL)
	    ...
	    pr->pr_flags = 0;
	    pr->pr_pno = pr_getpno();
	    strncpy (pr->pr_name, process, SZ_PATHNAME+1);
	    pr->pr_name[SZ_PATHNAME] = '\0';
	}
 ...
 */
void pr_pdisconnect ( struct process *pr )
{
...
void pr_setcache ( int new_szprcache )
{
	...
	if (pr_head == NULL)
	     ...
	     */
	    for (pr=pr_head;  pr != NULL;  pr=pr->pr_dn) {
		if ( MAXSUBPROC <= nprocs ) {
		    cl_error (E_UERR, "pr_setcache: too large nprocs");
		}
 ...
 * processes.
 */
struct process *pr_findproc ( const char *process )
{
	register struct	process *pr;
 ...
 */
/* ltname : logical task name	*/
int pr_cachetask ( const char *ltname )
{
	register int /*n,*/ pid;
	struct	ltask *ltp;
	FILE	*fdummy;
...
void pr_listcache ( FILE *fp )
{
	...
	for (pr=pr_head;  pr != NULL;  pr=pr->pr_dn)
		 ...
		 */
		os_pid = c_kimapchan (pr->pr_pid, nodename, SZ_FNAME+1);
		snprintf (out, 100, "[%02d] %s!%d(%xX)",
...
void pr_dumpcache ( int pid, int break_locks )
{
	 ...
	 */
	for ( pr=pr_cache, n=sz_prcache ; n > 0 ; pr++, n-- )
 ...
 */
int pr_pnametopid ( const char *pname )
{
	register struct process *pr;
 ...
 */
void pr_chdir ( int pid, const char *newdir )
{
	register struct process *pr;
 ...
 */
void pr_envset ( int pid, const char *envvar, const char *valuestr )
{
	register struct process *pr;
...
void onipc ( XINT *vex, void (**next_handler)() )
{
	register struct	process *pr;
...
void pr_initcache( void )
{
	...
	for ( pr=pr_cache, n=MAXSUBPROC ; n > 0 ; pr++, n-- ) {
	    pr->pr_pid = 0;
	    pr->pr_flags = 0;
	    pr->pr_pno = 0;
 ...
 */
void pr_tohead ( struct process *pr )
{
	if (pr_head != pr) {
	    pr_unlink (pr);
	    pr->pr_dn = pr_head;
 ...
 */
void pr_totail ( struct process *pr )
{
	if (pr_tail != pr) {
	    pr_unlink (pr);
	    pr->pr_up = pr_tail;
 ...
 */
void pr_unlink ( struct process *pr )
{
	if (pr->pr_up) {
