...
static char *op_cmdblk=cmdblk;	/* next output line in cmdblk		*/
static int cmdblk_line=0;	/* line number within cmd block		*/
static int cmdblk_save=0;	/* set if cmdblk filled interactively	*/
static char raw_cmdblk[SZ_CMDBLK+1];	/* saves raw command for history (for scripts) */

static char histbuf[SZ_HISTBUF+1];	/* history buffer		*/
static char *op_hist=histbuf;	/* next location in history buffer	*/
static int histbuf_full=0;	/* set when buffer wraps around		*/
static int share_logfile=SHARELOG;	/* share logfile with other processes?	*/

static FILE *logfp=NULL;	/* file pointer for command logfile	*/
static int histnum = 0;		/* history command block number		*/
static int history_number;	/* the current history record		*/


static int get_command ( FILE * );
static int search_history ( const char *, char *, int );
static int stredit ( const char *, const char *, char *, int );
static int expand_history_macros ( const char *, char *, int );
static int get_arglist ( const char *, char *, char *[], int );
static void fetch_history ( const char *, char *, int );
static char *find_history ( int );
static void make_prompt( const char *, char *, int );
#ifdef NOREADLINE
static void pprompt ( const char * );
#endif
static void put_logfile ( const char * );
static void put_cmdlogfile ( const char * );
static void print_command ( FILE *, const char *, const char *, const char * );
 ...
 */
static int get_command ( FILE *fp )
{
	register char *ip, *op;
	char	*maxop;
	...
	if (!(currentask->t_flags & T_INTERACTIVE)  ||
		...
		status = (fgets (cmdblk, SZ_CMDBLK+1, fp) == NULL ? EOF : OK);
		if (status == EOF) {
		    cmdblk[0] = '\0';
		    break;
		}
...
input_:
		...
		char prompt[SZ_LINE];
		if (cmdblk_line == 0) {
		    make_prompt(curpack->pk_name,prompt,SZ_LINE);
		    rl_buf = readline (prompt);
		}
		else {
		    make_prompt(NOCLOSURE,prompt,SZ_LINE);
		    rl_buf = readline (prompt);
		}
		if ( rl_buf == NULL ) {
		    raw_cmd[0] = '\0';
		    ...
		    strncpy(raw_cmd,rl_buf,SZ_LINE-1);
		    raw_cmd[SZ_LINE-1-1]='\0';
		    if ( raw_cmd[0] != '\0' ) add_history(raw_cmd);
		    strcat(raw_cmd,"\n");
		    rl_free(rl_buf);
		}
	    }
	...
	if (*raw_cmd == HISTCHAR) {
	     ...
	     */
	    if (eh_verify)
		execute = edit_history_directive (raw_cmd+1, new_cmd, SZ_CMDBLK+1);
	    else {
		execute = process_history_directive (raw_cmd, new_cmd, SZ_CMDBLK+1);
		fputs (new_cmd, currentask->t_stdout);
	    }

	} else if (expand_history_macros (raw_cmd, new_cmd, SZ_CMDBLK+1)) {
	    fputs (new_cmd, currentask->t_stdout);

	} else {
	    static const char *ehist = "ehistory";
	    int     n;

	    n = 0;
	    for ( ip=raw_cmd ; (*ip) && ehist[n] && (*ip == ehist[n]) ; ip++ )
		n++;
	    if (n > 0 && isspace (*ip)) {
		while (isspace (*ip))
		    ip++;
		execute = edit_history_directive (ip, new_cmd, SZ_CMDBLK+1);
	    }
	}
	...
	maxop = cmdblk + SZ_CMDBLK+1 -1;
	for ( ip=new_cmd ; op < maxop && *ip != EOS ; op++, ip++ )
	    *op = *ip;
	if ( op <= maxop ) *op = EOS;
 ...
 */
int process_history_directive ( const char *directive, char *new_command_block,
				int bufsize )
{
	register char *ip, *op, *p;
	char	*maxop;
	char	last_command_block[SZ_CMDBLK+1];
	char	ac_directive[SZ_CMDBLK+1];
	int	execute=1, edit=0;
	int	record;

	strncpy(ac_directive,directive,SZ_CMDBLK+1);
	ac_directive[SZ_CMDBLK] = EOS;

	ip = ac_directive + 1;			/* skip the '^'		*/
	op = new_command_block;
	maxop = new_command_block + bufsize -1;
	...
	if (edit) {
	    ...
	    if (get_history (1, last_command_block, SZ_CMDBLK+1) == ERR)
		cl_error (E_UERR, "Nothing in history buffer to edit");
	    ip = ac_directive +
		stredit (ac_directive, last_command_block, new_command_block,
			 bufsize);
	...
	/* Directives "^absnum" and "-relnum". */
	    ...
	    if (get_history (record, new_command_block, bufsize) == ERR)
	...
	/* Directives "^", "^str", and "^?str".  */
	    ...
	    ip = ac_directive + search_history (ac_directive, new_command_block, bufsize);
	...
	while (isspace (*(op-1)))
	    --op;
	expand_history_macros (ip, op, (maxop - op) + 1);
 ...
 * matches anything.
 */
static int search_history ( const char *directive, char *new_command_block, 
			    int bufsize )
{
	const char *ip;
	char	*op, *p;
	...
	maxop = pattern + SZ_FNAME -1;
	for ( op=pattern ; op < maxop && *ip != EOS ; op++, ip++ ) {
	    *op = *ip;
	...
	*op = EOS;
	...
	while (get_history (record++, new_command_block, bufsize) != ERR) {
	    if (patlen == 0) {
		break;
 ...
 */
/* edit_directive : e.g., "^str1^str2^"		*/
/* in_text        : text to be edited		*/
/* out_text       : buffer for output text	*/
static int stredit ( const char *edit_directive, const char *in_text, 
		     char *out_text, int bufsize )
{
	const char *ip;
	char	*op, *pp;
	char	*maxop;
	...
	maxop = pattern + SZ_LINE+1 -1;
	for ( op=pattern ; op < maxop && *ip != EOS ; ip++, op++ ) {
	    *op = *ip;
	...
	*op = EOS;
	 ...
	 */
	if (patlen > 0) {
	    maxop = text + SZ_LINE+1 -1;
	    for ( op=text ; op < maxop && *ip != EOS ; ip++, op++ ) {
		*op = *ip;
	...
	maxop = out_text + bufsize -1;
	for (ip=in_text, op=out_text;  *ip != EOS;  ) {
	    /* Advance to next match */
	    for ( pp=pattern ; op < maxop && *ip != EOS ; op++, ip++ ) {
		*op = *ip;
	     ...
	     */
	    if (*ip == *pp) {
		for ( pp=text ; op < maxop && *pp != EOS ; op++, pp++ )
		    *op = *pp;
		ip += patlen;
	    }

	    if (!replace_all_occurrences) {
		while ( op < maxop && *ip != EOS ) {
		    *op = *ip;
		    op++;
		    ip++;
		}
 ...
 */
static int expand_history_macros ( const char *in_text, char *out_text, 
				   int bufsize )
{
	const char *ip;
	char	*op, *ap;
	char	*maxop;
	char	cmdblk[SZ_CMDBLK+1], arg_cmdblk[SZ_CMDBLK+1], *argp[100];
	 ...
	 */
	maxop = out_text + bufsize -1;
	for ( ip=in_text, op=out_text ; op < maxop && (*ip) != EOS ; ip++, op++ ) {
	    *op = *ip;
	    if (*ip == HISTCHAR) {
		if (ip > in_text && *(ip-1) == '\\') {
		    *(--op) = HISTCHAR;				/* \^	*/
		 ...
		 */
		if (!have_arg_strings++) {
		    if (get_history (1, cmdblk, SZ_CMDBLK+1) == ERR)
			cl_error (E_UERR, "Nothing in history buffer");
		    nargs = get_arglist (cmdblk, arg_cmdblk, argp, 100);
		}
		 ...
		 */
		for ( ; nrep > 0 && argno <= nargs ; nrep-- ) {
		    for ( ap=argp[argno++] ; op < maxop && (*ap) ; op++, ap++ )
			*op = *ap;
		    if (nrep > 1)
			*op++ = ' ';
		}
 ...
 */
/* arg_cmdblk : buffer to store argument list in	*/
/* argp       : receives argument pointers		*/
static int get_arglist ( const char *cmdblk, char *arg_cmdblk, char *argp[], 
			 int argpsize )
{
	register char	*cp;
	register int	nargs;

	strcpy(arg_cmdblk,cmdblk);

	for (cp=arg_cmdblk, nargs=0;  *cp != EOS;  ) {
 ...
 */
void put_history ( const char *command )
{
	register char *ip, *op, *otop;
	char ac_cmd[SZ_CMDBLK+1];

	strncpy(ac_cmd,command,SZ_CMDBLK+1);
	ac_cmd[SZ_CMDBLK+1 - 1] = EOS;

	/* Make sure there is exactly one newline at the end of the command. */
	otop = ac_cmd + SZ_CMDBLK+1 -1;
	for ( op = ac_cmd + strlen(ac_cmd) ; ac_cmd < op ; ) {
	    op--;
	    if (!isspace (*op)) break;
	}
	*++op = '\n';
 ...
 */
int get_history ( int record, char *command, int bufsize )
{
	char	*recptr;
	...
	if ((recptr = find_history (record)) == NULL) {
	    *command = EOS;
	    ...
	    fetch_history (recptr, command, bufsize);
	    return (OK);
	}
}
 ...
 */
static void fetch_history ( const char *recptr, char *command, int bufsize )
{
	const char	*ip;
	register char	*op, *maxop, *itop;
	register int	n;

	itop = histbuf + SZ_HISTBUF;
	ip   = recptr;
	op   = command;
	n    = ((bufsize < SZ_HISTBUF+1) ? bufsize : SZ_HISTBUF+1);
	maxop = op + n - 1;
 ...
 */
static char *find_history ( int record )
{
 ...
 */
void show_history ( FILE *fp, int max_commands )
{
 ...
 */
static void make_prompt( const char *string, char *prompt, int bufsize )
{
	...
	snprintf (prompt, bufsize, "%2.2s> ", string);
}

#ifdef NOREADLINE
static void pprompt ( const char *string )
{
	char prompt[SZ_LINE];
	make_prompt(string,prompt,SZ_LINE);
	printf("%s",prompt);
	fflush (stdout);
}
#endif
 ...
 */
static void put_logfile ( const char *command )
{
	FILE	*fp;
...
static void put_cmdlogfile ( const char *command )
{
	char *op;
	char *maxop;
	char ac_cmd[SZ_CMDBLK+1];
 ...
 */
void close_logfile ( const char *fname )
{
	register FILE *fp;
 ...
 */
/* marg1 : margin strings of first and subseq. cmds */
static void print_command ( FILE *fp, const char *command, const char *marg1, 
			    const char *marg2 )
{
	const char *ip;
 ...
 * CL (e.g., exec.c).
 */
/* tp : pointer to task or NULL */
void putlog ( struct task *tp, const char *usermsg )
{
	const char	*ip;
	register char	*op, *otop;
	register int	n;
	char	msg[SZ_LOGBUF], job[5];
	const char	*pkg, *tname;
	 ...
	 */
	snprintf (msg, SZ_LOGBUF, "# %8.8s %s%s%s %s- ",
		(today() + 4), pkg, (tp ? "." : ""), tname, job);
	for ( op=msg, n=0 ; (*op) ; op++ )
	    n++;
	otop = msg + SZ_LOGBUF -1;
	for ( ip=usermsg ; op < otop && (*ip) ; n++ ) {
	    *op++ = *ip++;
	    if (n + 2 >= MAXCOL) {
		if ( op < otop ) *op++ = '\\';
		if ( op < otop ) *op++ = '\n';
