...
static int nargs ( struct pfile * );
static void pushbparams ( struct param * );
static void do_clprint ( const char * );
static void pushbpvals ( struct param * );
static void pushfparams ( struct param * );
static int mkarglist ( struct pfile *, char *, int, char *[], int );
static void newbuiltin ( struct package *, const char *, void (*)(), int, 
			 const char *, int );
...
void cllogout( void )
{
	...
	if (nallocdev > 0) {
		...
		dv = &allocdev[n];
		if (dv->allocated)
		    if (c_devowner(dv->devname,owner,SZ_FNAME+1) != DV_DEVALLOC) {
			dv->allocated = NO;
			--nallocdev;
		    }
	    }
...
void clcache ( void )
{
	...
	if ((n = nargs (pfp)) < 1) {
	    ...
	    pfp = pfp->pf_npf)
		if (!(pfp->pf_flags & (PF_FAKE|PF_COPY))) {
		    snprintf(pfilename, SZ_PATHNAME, "%s.%s",
			     pfp->pf_ltp->lt_pkp->pk_name,
			     pfp->pf_ltp->lt_lname);
 ...
 */
void cl_locate ( const char *task_spec, int first_only )
{
	char	buf[SZ_LINE];
	...
	if (pkname[0] != '\0') {	/* explicit package named	*/
	    ...
	    if ((stat = (long) ltaskfind (pkp, ltname, 1)) == 0)
		oprintf ("%s'\n", pkname);
...
static	const char *cd_emsg = "Cannot change directory to `%s'";
...
void clerror( void )
{
	...
	if (keeplog() && log_errors())
		...
		snprintf(buf, SZ_LINE, "ERROR: %s", errmsg);
...
static void do_clprint ( const char *dest )
{
...
void clstty( void )
{
	 ...
	 */
	argc = mkarglist (pfp, args, 1024, argp, 100);

	/* Concatenate the stty argument list.  */
	maxop = sttycmd + 2048 -1;
	for (op=sttycmd, i=1;  i <= argc;  i++) {
	    for ( ip=argp[i] ; op < maxop && (*ip) ; op++, ip++)
		*op = *ip;
	    if (i < argc && op < maxop)
		*op++ = ' ';
	}
	*op = EOS;
	...
	c_sttyco (sttycmd, STDIN, STDOUT, fileno(newtask->t_stdout));
	keep (prevtask);
}
...
void clforeign( void )
{
	...
	maxop = oscmd + 1024 -1;
	op = oscmd;
	for ( ip=newtask->t_ltp->lt_ftprefix ; op < maxop && (*ip) ; op++,ip++ ) {
	    if (*ip == '\\' && *(ip+1) == '$')
		*op = *(++ip);
	    else if (*ip == '$') {
		dolseen++;
		...
		for (n=n1;  n <= n2;  n++) { 
			...
			if ( n > n1 && op <= maxop )
			    *op++ = ' ';
			if (mapfname) {
			    c_fmapfn (argp[n], osfn, SZ_PATHNAME+1);
			    ap = osfn;
			} else
			    ap = argp[n];
			while ( *ap ) {
	...
	*op = EOS;
	...
	if (!dolseen) {
	    for (n=1;  n <= argc;  n++) {
		if ( op < maxop ) *op++ = ' ';
		for ( ap=argp[n] ; op < maxop && (*ap) ; op++,ap++ ) {
		    *op = *ap;
		}
	    }
	    *op = EOS;
...
void clunlearn( void )
{
	static const char *errfmt = "Warning: Cannot unlearn params for `%s'\n";
	register struct pfile *pfp;
...
void cledit( void )
{
	...
	if ((n = nargs(pfp)) > 0) {
		...
		if (os_filelist[0] != EOS) {
		    snprintf (os_filelist+strlen(os_filelist),
			      SZ_LINE-strlen(os_filelist), " %s", osfn);
		} else {
		    snprintf (os_filelist, SZ_LINE, "%s", osfn);
		}
	    }
	}
	 ...
	 * to edit the file(s).
	 */
	snprintf (oscmd, SZ_LINE,
		  host_editor (envget ("editor")), os_filelist);
	c_oscmd (oscmd, "", "", "");
}
...
void clallocate( void )
{
	register struct pfile *pfp;
	register int	n;
	static const char *noalloc = "cannot allocate device %s";
	struct	operand o;
	...
	o = popop();
	strncpy (device, o.o_val.v_s, SZ_FNAME+1);
	device[SZ_FNAME] = EOS;

	/* Verify that the device can be allocated.
	 */
	switch (c_devowner (device, owner, SZ_FNAME+1)) {
	case DV_DEVFREE:
...
void cldeallocate( void )
{
	register struct pfile *pfp;
	register int	n;
	static const char *nodealloc = "cannot deallocate device %s";
	struct	operand o;
	char	device[SZ_FNAME+1];
	char	owner[SZ_FNAME+1];
	int	rewind=0, n_args;
 ...
 */
void setbuiltins ( struct package *pkp )
{
 ...
 */
/* pkp      : package which owns task	*/
/* lname    : ltask name		*/
/* fp       : pointer to builtin fcn	*/
/* flags    : task flags		*/
/* ftprefix : OSCMD prefix if foreign	*/
/* redef    : permit redefinitions	*/
static void newbuiltin ( struct package *pkp, const char *lname, void (*fp)(),
			 int flags, const char *ftprefix, int redef )
{
 ...
 */
/* pfp  : pfile pointer			*/
/* args : string buffer for arg chars	*/
/* argp : array of arg pointers		*/
static int mkarglist ( struct pfile *pfp, char *args, int size_args, 
		       char *argp[], int size_argp )
{
	register char	*ip, *op, *maxop;
	...
	if ((argc = nargs(pfp)) > 0) {
	    pushbparams (pfp->pf_pp);
	    op = args;
	    maxop = args + size_args -1;

	    if ( n < size_argp-1 )
		argp[n] = newtask->t_ltp->lt_lname;

	    for ( n=1 ; n < size_argp-1 && n <= argc ; n++ ) {
		argp[n] = op;
 ...
 */
static void pushfparams ( struct param *pp )
{
	struct operand onam;
 ...
 */
static void pushbparams ( struct param *pp )
{
	struct operand onam;
	struct param *npp;
 ...
 */
static void pushbpvals ( struct param *pp )
{
	/* struct operand onam; */
	struct param *npp;
 ...
 */
static int nargs ( struct pfile *pfp )
{
	struct	param	*pp;
	int	n;
