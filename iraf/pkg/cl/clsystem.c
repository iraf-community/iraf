...
/* taskout : stdout of task		*/
/* taskerr : stderr of task		*/
void clsystem ( const char *cmd, FILE *taskout, FILE *taskerr )
{
	const char	*ip;
	register int	ch;
	...
	if (taskerr == taskout) {
	    strncpy (errfile, outfile, SZ_PATHNAME);
	    errfile[SZ_PATHNAME-1] = EOS;
	}
