...
#define YYLMAX BUFSIZ	/* in lexyy.c */
...
static int lexdebug=0;		/* debug lexical analyzer		*/
static int lexcol=0;		/* nchars since \n or ;			*/
static int pbtoken;		/* push back token			*/
static int newarg;		/* whitespace argument delimiter seen	*/
static int lhs;			/* "left hand side" switch for []	*/

static int lexicon( void );
...
static int lexicon( void )
{
	const char *bkgerr = "ERROR: cannot submit background job inside {}\n";
	register int	ch, cch;
	register int	token;
...
etok_:		if (!newtoken) {
		...
		while ((ch = input()) && ch != '\n') {
			...
			if (ch = input()) {
				...
				if ( YYLMAX - 1 <= yyleng ) 
				    cl_error (E_UERR, "Too large yyleng.");
				yytext[yyleng++] = '\\';
			    }
			} else
			    break;
		...
		if (!newtoken) {
			...
			if (ch == 0) {
			    ...
			    if ( op < maxop ) *op++ = '>';
			    if ( op < maxop ) *op++ = ch;
			    else cl_error (E_UERR, "Too large yyleng.");
			    goto gsredir_;
			} else if (ch == '&') {
			    token = Y_ALLAPPEND;
			    goto eatwhite_;
