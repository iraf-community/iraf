...
static char bkgmsg[SZ_BKGMSG+1];	/* passed to kernel		*/
...
static void rbkgfile ( const char * );
static char *wbkgfile ( int, const char * );
 ...
 */
/* bcs : background control string	*/
void bkg_init ( const char *bcs )
{
	strncpy (bkgmsg, bcs, SZ_BKGMSG);
	bkgmsg[SZ_BKGMSG]=EOS;
	dobkg++;
}
 ...
 * seen an '&'.
 */
/* cmd : command entered by user to spawn job	*/
void bkg_spawn ( const char *cmd )
{
	...
	if (keeplog() && log_background()) {
	    char  buf[SZ_LINE];
	    snprintf (buf, SZ_LINE, "Start [%d]", jobno);
	    putlog (0, buf);
	}
}
...
/* fp  : output file		*/
/* job : job(s)			*/
void bkg_jobstatus ( FILE *fp, int job )
{
	register struct _bkgjob *bk;
	register int	j;
	register char	*ip, *maxip;
	long	seconds;
	char	outstr[SZ_LINE];
	...
	for (bk=jobtable, j=1;  j <= NBKG;  j++, bk++)
		...
		if (busy(j)) {
			...
			snprintf(outstr,SZ_LINE,"%s","Stopped");
		    else
			snprintf(outstr,SZ_LINE,"%s","Running");
		} else if (bk->b_flags & J_KILLED) {
		    snprintf(outstr,SZ_LINE,"%s","Killed");
		} else if (bk->b_exitcode == OK) {
		    snprintf(outstr,SZ_LINE,"%s","Done");
		} else
		    snprintf (outstr, SZ_LINE, "Exit %d", bk->b_exitcode);
		fprintf (fp, "%-10s", outstr);

		/* Finally, print user command followed by newline.
		 */
		ip = bk->b_cmd;
		maxip = ip + c_envgeti ("ttyncols") - (8 + 8 + 10) - 1 -1;
		for ( ; ip <= maxip && *ip != EOS ; ip++ )
		    if (*ip == '\n' || *ip == '\t')
			fputc (' ', fp);
		    else
			fputc (*ip, fp);
 ...
 */
/* job  : job ordinal			*/
/* pmsg : print termination message	*/
void bkg_close ( int job, int pmsg )
{
	...
	if (keeplog() && log_background()) {
	    char  buf[SZ_LINE];
	    snprintf (buf, SZ_LINE, "Stop [%d]", job);
	    putlog (0, buf);
	}
}
...
int bkg_wfservice ( int job )
{
	...
	get_bkgqfiles (job, c_getpid(), 
		       bkg_query_file, SZ_PATHNAME,
		       query_response_file, SZ_PATHNAME);
 ...
 */
void bkg_startup ( const char *bkgfile )
{
	rbkgfile (bkgfile);
	setclmodes (firstask);
 ...
 */
/* jobno : ordinal jobnumber of child	*/
/* cmd   : command to be run in bkg	*/
static char *wbkgfile ( int jobno, const char *cmd )
{
	static	const char *bkgwerr = "error writing background job file";
	 ...
	 * dictionary.
	 */
	if (fwrite ((const void *)&bh, BKGHDRSIZ, 1, fp) == 0)
	    cl_error (E_IERR|E_P, bkgwerr);
	if (fwrite ((const void *)stack, STACKSIZ, BPI, fp) == 0)
	    cl_error (E_IERR|E_P, bkgwerr);
	if (fwrite ((const void *)dictionary, topd, BPI, fp) == 0)
	    cl_error (E_IERR|E_P, bkgwerr);
 ...
 */
static void rbkgfile ( const char *bkgfile )
{
	char	set[SZ_ENVDEF];
	struct	bkgfilehdr bh;
	int	n;
	...
	if (fread ((void *)&bh, BKGHDRSIZ, 1, fp) == 0)
	    goto abort_;
	 ...
	 */
	if (fread ((void *)stack, bh.b_szstack, 1, fp) == 0)
	    goto abort_;
	if (fread ((void *)dictionary, bh.b_szdict, 1, fp) == 0)
	    goto abort_;
