...
static int comstr ( const char *, memel *, int );
...
int compile ( int opcode, ... )
{
	...
	case SWOFF:
 	case SWON: {
		const char *sp;
		sp = va_arg (argp, const char *);
		status = comstr (sp, &cep->c_args, maxpc);
		if (status != ERR)
		    cep->c_length += status;
		}
		break;
	...
	case PUSHCONST: {
		...
		*dp = *op;
		...
		if ((op->o_type & OT_BASIC) == OT_STRING) {
		    status = comstr (op->o_val.v_s, argsaddr, maxpc);
		    if (status != ERR) {
 ...
 */
static int comstr ( const char *s, memel *loc, int maxpc )
{
	register char *to, *from, *maxto;
	register struct codeentry *maxcep;
	maxcep = coderef(maxpc);
	maxto = (char *)maxcep + sizeof(*maxcep) -1;
 ...
 * originally.
 */
void catdstr ( char *es, const char *ns )
{
