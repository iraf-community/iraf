...
static int qstrcmp ( const void *, const void * );
 ...
 */
void prparamval ( struct param *pp, FILE *fp )
{
	char	buf[SZ_LINE];

	spparval (buf, SZ_LINE, pp);
	fputs (buf, fp);
}
...
void strsort ( char *list[], int nstr )
{
...
/* list                : array of string pointers		*/
/* nstr                : number of strings			*/
/* first_col, last_col : where to place table on a line		*/
/* maxch               : maximum chars to print from a string	*/
/* ncol                : desired # of columns (0 to autoscale)	*/
void strtable ( FILE *fp, char *const list[], int nstr, int first_col,
		int last_col, int maxch, int ncol )
{
