...
static char *strint ( const char *s, int side )
{
	if (side == LEFTSIDE) {
	    while (isdigit (*s))
		s++;
...
void binop ( int opcode )
{
	...
	if (typ1 == OT_STRING || typ2 == OT_STRING || typecode == OT_STRING) {
		...
		if (typ2 == OT_STRING) {
		    snprintf ( res, 2*SZ_LINE, "%s%s", o1sp, o2.o_val.v_s );
			 ...
			 */
			strncpy (res, o1sp, 2*SZ_LINE < cp - o1sp ? 2*SZ_LINE : cp - o1sp);
			res[2*SZ_LINE-1] = EOS;
			newnum = atoi(cp) + (int)VALU(&o2);
			snprintf (format, MAX_DIGITS, "%%0%dd", strlen (cp));
			if ( cp - o1sp < 2*SZ_LINE ) {
			    snprintf ((char *)(res + (cp - o1sp)),
				      2*SZ_LINE - (cp - o1sp), format, newnum);
			}
			if (newnum < 0)
			    cl_error (E_UERR,
			...
			snprintf ( res, 2*SZ_LINE, "%s%d", o1sp, (int)VALU(&o2) );
		    }
		}
		break;
		 ...
		 */
			 ...
			 */
			if (typ2 == OT_STRING) {
			    strncpy (s2, o2.o_val.v_s, SZ_LINE);
			    s2[SZ_LINE-1] = EOS;
			}
			pushop (&o1);
			opcast (OT_STRING);
		    ...
		    strncpy (res, o1.o_val.v_s, 2*SZ_LINE);
		    res[2*SZ_LINE-1] = EOS;
		     ...
		     */
		    if (typ1 != OT_STRING && typ2 == OT_STRING)
			snprintf (res, 2*SZ_LINE, "%s%s", o1.o_val.v_s, s2);
		    else
			snprintf (res, 2*SZ_LINE, "%s%s", o1.o_val.v_s, o2.o_val.v_s);
		...
		snprintf (res, 2*SZ_LINE, "%r*", o2.o_val.v_i, lval);
		break;
