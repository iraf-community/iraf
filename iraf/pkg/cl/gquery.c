...
static const char *e1 = "Not in batch";
static const char *e2 = "Parameter value is out of range";
 ...
 */
const char *gquery ( struct param *pp, const char *string )
{
	register char *ip;
	char	buf[SZ_LINE];
	char	*query_status, *nlp;
	const char *errmsg;
	...
	if (batch) {
	    ...
	    query_status = strncpy (buf, string, SZ_LINE);
	    buf[SZ_LINE-1] = EOS;
	}
...
char *minmax ( struct param *pp )
{
	...
	if (range_check (pp)) {
		...
		o.o_val = pp->p_min;
		sprop (str, SZ_LINE, &o);
		strncat(message, str, SZ_LINE-1-strlen(message));
	    }
	    strncat(message, ": maximum=", SZ_LINE-1-strlen(message));
	    if (!(pp->p_flags & (P_IMAX|P_UMAX))) {
		o.o_val = pp->p_max;
		sprop (str, SZ_LINE, &o);
		strncat(message, str, SZ_LINE-1-strlen(message));
	    }
	    strncat(message, ")", SZ_LINE-1-strlen(message));
	} else
	    message[0] = EOS;
