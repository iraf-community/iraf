...
begin
	...
	do i = 1, nregions {
	    ...
	    xin = (Meml[prc1+i-1] + Meml[prc2+i-1]) / 2.0
	    yin = (Meml[prl1+i-1] + Meml[prl2+i-1]) / 2.0
	    ...
	    c1 = Meml[prc1+i-1] + xlag
	    c2 = Meml[prc2+i-1] + xlag
	    l1 = Meml[prl1+i-1] + ylag
	    l2 = Meml[prl2+i-1] + ylag

	    if (IS_INDEFR(Memr[pxshift+i-1]) || IS_INDEFR(Memr[pyshift+i-1])) {
		if (dformat == YES)
	            call rg_xdbshiftr (db, Meml[prc1+i-1], Meml[prc2+i-1],
		        Meml[prl1+i-1], Meml[prl2+i-1], c1, c2, l1, l2,
		        INDEFR, INDEFR)
	    } else {
		if (dformat == YES)
	            call rg_xdbshiftr (db, Meml[prc1+i-1], Meml[prc2+i-1],
...
pointer	db		#I pointer to the database file
long	rc1, rc2	#I reference region column limits
long	rl1, rl2	#I reference region line limits
long	c1, c2		#I image region column limits
long	l1, l2		#I image region line limits
...
begin
	...
	call printf ("Region %d: [%d:%d,%d:%d]  [%d:%d,%d:%d]  %g  %g\n")
	    call pargi (i)
	    call pargl (Meml[prc1+i-1])
	    call pargl (Meml[prc2+i-1])
	    call pargl (Meml[prl1+i-1])
	    call pargl (Meml[prl2+i-1])
	    call pargl (c1)
	    call pargl (c2)
	    call pargl (l1)
	    call pargl (l2)
