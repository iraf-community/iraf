...
size_t	sz_val
long	i, j
int	k, l, n1, n2, n3, n4, n5, id, nl, nh, nin, maxkeep
bool	even
real	sigma, r, s, t
...
begin
	...
	if (dflag == D_ALL) {
	    ...
	    if (pclip < 0.) {
		if (even)
		    n3 = max (1, inint(n2 - 1 + pclip))
		else
		    n3 = max (1, inint(n2 + pclip))
	    } else
		n3 = min (n1, inint(n2 + pclip))
	    nin = n1
	}
...
procedure ic_pclipl (d, m, n, nimages, npts, median)
...
begin
	...
	do i = 1, npts {
	    ...
	    for (nl=1; nl<=n1; nl=nl+1) {
		r = (med - Meml[d[nl]+j]) / sigma
		if (r < lsigma)
		    break
		Memr[resid+nl] = r
	    }
	    for (nh=n1; nh>=1; nh=nh-1) {
		r = (Meml[d[nh]+j] -  med) / sigma
		if (r < hsigma)
		    break
		Memr[resid+nh] = r
	    }
