...
size_t	sz_val
long	i, k
int	j, l, jj, n1, n2, nin, nk, maxkeep
...
size_t	sz_val
long	i, k
int	j, l, id, n1, n2, n3, nl, nh, nin, maxkeep
real	r, s
...
procedure ic_asigclipl (d, m, n, scales, zeros, nimages, npts, average)
...
procedure ic_msigclipl (d, m, n, scales, zeros, nimages, npts, median)
...
begin
	...
	do i = 1, npts {
		...
		if (n1 == 0)
		    ...
		    med = (Meml[d[n3-1]+k] + Meml[d[n3]+k]) / 2.
		else
		    med = Meml[d[n3]+k]
		...
		if (n1 >= max (MINCLIP, maxkeep+1)) {
			...
			if (s > 0.) {
			    for (; nl <= n2; nl = nl + 1) {
				r = (med - Meml[d[nl]+k]) / (s * Memr[w+nl-1])
				if (r <= lsigma)
				    break
				Memr[resid+nl] = r
				n1 = n1 - 1
			    ...
			    for (; nh >= nl; nh = nh - 1) {
				r = (Meml[d[nh]+k] - med) / (s * Memr[w+nh-1])
				if (r <= hsigma)
				    break
				Memr[resid+nh] = r
				n1 = n1 - 1
