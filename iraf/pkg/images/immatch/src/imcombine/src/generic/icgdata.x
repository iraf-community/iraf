...
size_t	sz_val
short	temp
int	i, l, ndim, nused, mtype
long	j, k, n1, n2
size_t	npix, nin, nout
real	a, b
...
procedure ic_gdatal (in, out, dbuf, d, id, n, m, lflag, offsets, scales,
	zeros, nimages, npts, v1, v2)
...
pointer	buf, dp, ip, mp
long	xt_imgnll()
errchk	xt_cpix, xt_imgnll

long	max_pixel
data	max_pixel/MAX_LONG/
...
begin
	...
	do i = 1, nimages {
		...
		if (project)
		    v2[ndim+1] = i
		j = xt_imgnll (in[i], i, d[i], v2, v1[2])
	    } else {
		nin = IM_LEN(in[i],1)
		j = max (0, offsets[i,1])
		k = min (nout, nin + offsets[i,1])
		...
		if (project)
		    v2[ndim+1] = i
		l = xt_imgnll (in[i], i, buf, v2, v1[2])
		call amovl (Meml[buf+k-1], Meml[dbuf[i]+j], npix)
		d[i] = dbuf[i]
	    }
	}
	...
	if (mclip) {
	    sz_val = nused
	    call malloc (dp, sz_val, TY_LONG)
	    if (keepids) {
		sz_val = nused
		call malloc (ip, sz_val, TY_INT)
		call ic_2sortl (d, Meml[dp], id, Memi[ip], n, npts)
		call mfree (ip, TY_INT)
	    } else
		call ic_sortl (d, Meml[dp], n, npts)
	    call mfree (dp, TY_LONG)
	}
