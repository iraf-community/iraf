...
int	btype, ndim
size_t	nxvals, nyvals, nzvals, width
...
begin
	...
	if (ndim == 1)
	    ...
	    nyvals = IM_LEN(im,2)
	call clputl ("x1.p_maximum", nxvals)
	call clputl ("x2.p_maximum", nxvals)
	call clputl ("y1.p_maximum", nyvals)
	call clputl ("y2.p_maximum", nyvals)
	...
	nzvals = lint(sqrt ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1))) + 1
...
size_t	nzvals, width					#I Plot parameters
...
size_t	width					#I Width of sampled points
...
begin
	...
	l_val = 1
	call amovr (y, Memr[impl2r(imo,l_val)], npts)
...
size_t	nxvals, nyvals		# image dimensions
...
real	x1, y1			# starting point of vector
real	theta			# angle of vector (ccw from +x)
size_t	nxvals, nyvals		# size of image
real	x2, y2			# point where vector intersects boundary
...
long	i, j, k
long	nedge, col1, col2, line1, line2
...
begin
	...
	# Set the boundary.
	col1 = lint(min (x1, x2)) - nedge
	col2 = lnint(max (x1, x2)) + nedge
	line1 = lint(min (y1, y2)) - nedge
	...
	do i = 1, nvals { 
		...
		colb = max (col1, lint(min (lim1, lim2, lim3, lim4)) - 1)
...
begin
	...
	col2 = lnint(max (x1, x2)) + nedge
	l_val = 1
	call pv_setboundary (im, col1, col2, l_val, l_val, btype, bconstant)
