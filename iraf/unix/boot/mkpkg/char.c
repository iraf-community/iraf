...
static void mk_pbbuf ( struct context * );
	...
	while ((ch = m_rawgetc (cx)) == '$') {
	    ...
	    *op = EOS;
		...
		if ((fp = fopen (&name[1], "r")) == NULL) {
		    ...
		    maxop = lbuf + SZ_CMD+1 -1;
		    for ( op=lbuf ; op < maxop && (ch=getc(fp)) != EOF ; op++ ) {
			*op = isspace(ch) ? ' ' : ch;
		    }
		    while (op > lbuf) {
			ch = *(op-1);
 ...
 */
int m_rawgetc ( struct context *cx )
{
	struct pushback *pb;
	int ch;
 ...
 */ 
void m_pushstr ( struct context *cx, const char *str )
{
	struct pushback *pb;
	char *op, *otop;
	const char *ip;
	...
	if (debug > 2) {
	    if (str[0] <= 040)
		printf ("pushback punct char 0%lo\n", (unsigned long)str);
	    else
	...
	for ( ip=str, op=pb->op ; op < otop && (*ip) ; op++, ip++ ) {
	    *op = *ip;
	    if (*ip == '\n')
		--cx->lineno;
	}
	...
	if (debug > 2) {
	    ...
	    printf ("level=%d(%d), nleft=%ld, ip=%ld, op=%ld, bp=%ld, otop=%ld\n",
		    pb->npb, SZ_PBSTK, (long)(otop-op), 
		    (unsigned long)pb->ip, (unsigned long)pb->op, 
		    (unsigned long)pb->pbbuf, (unsigned long)otop);
	    fflush (stdout);
	}
 ...
 */
static void mk_pbbuf ( struct context *cx )
{
	struct pushback *pb;
 ...
 */
void pb_cancel ( struct context *cx )
{
	struct pushback *pb;
...
char *k_fgets ( char *obuf, size_t bufsize, struct context *cx )
{
	int ch;
	char *op, *maxop;
...
int k_fseek ( struct context *cx, long offset, int type )
{
	FILE *fp = cx->fp;
	int ch;
...
long k_ftell ( struct context *cx )
{
	if (debug > 1) {
...
char *k_fgets ( char *op, size_t bufsize, struct context *cx )
{
