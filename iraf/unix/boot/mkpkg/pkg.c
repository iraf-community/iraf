...
static int open_mkpkgfile ( struct context * );
static int search_mkpkgfile ( struct context * );
static int scan_modlist ( struct context *, int );
static void close_mkpkgfile ( struct context * );
static int up_to_date ( struct context *, const char *, const char *, 
			const char *[], int * );
static struct context *find_mkpkgfile ( struct context *, const char *, int );
...
int do_mkpkg ( struct context *cx, int islib )
{
	...
	if (open_mkpkgfile (cx) == ERR) {
	    ...
	    snprintf (fname, SZ_PATHNAME+1, "%s%s", cx->curdir, cx->mkpkgfile);
 ...
 */
/* cx : current mkpkg context	*/
static int scan_modlist ( struct context *cx, int islib )
{
	...
	if (islib && root_modlist) {
	     ...
	     * is required.
	     */
	    os_fpathname (cx->library, cx->libpath, SZ_PATHNAME+1);
	    if (debug) {
	 ...
	 */
	for (;;) {
next_:	    tok = gettok (cx, token, SZ_FNAME+1);
		...
		safe_strcpy (modname, SZ_PATHNAME+1, token);
		...
		safe_strcpy (srcname, SZ_PATHNAME+1, 
			     sfp ? sfp->sf_sfname : modname);
		if (sfp && debug) {
		 ...
		 */
		parse_fname (srcname, dname, fname, SZ_FNAME+1);
		...
		parse_modname (token, module, subdir, fname, SZ_FNAME+1);
		...
		if (islib && cx->nfiles == 0) {
			 ...
			 */
			maxop = dirname + SZ_FNAME+1 -1;
			for ( ip=cx->curdir, op=dirname ; op < maxop && (*ip) ; op++, ip++ )
			    *op = *ip;
			if ( dirname < op && *(op-1) == '/' ) op--;
			*op = EOS;
		    ...
		    if (exit_status != OK)
			fatals ("%s", "failed library update .");
		     ...
		     */
		    for (i=0;  i < cx->nrfiles;  i++) {
			parse_fname (cx->rflist[i], dname, fname, SZ_FNAME+1);
	...
	if (cx->totfiles == 0 && !forceupdate) {
		...
		fatals ("<ctrl/c> interrupt %s", cx->library);
	    if (exit_status != OK)
		fatals ("%s", "failed library rebuild .");
 ...
 */
/* modname : "module@subdir/fname"	*/
/* module  : receives module		*/
/* subdir  : receives subdir		*/
/* fname   : receives fname		*/
void parse_modname ( const char *modname, char *module, char *subdir, 
		     char *fname, size_t bufsize )
{
	 ...
	 */
	fname[0] = EOS;
	maxop = subdir + bufsize -1;
	for ( op=subdir, path=ip ; op < maxop && (*ip) ; op++, ip++ ) {
	    ch = *op = *ip;
	    if (ch == '$' || ch == '/') {
		if ( subdir < op && *(op-1) == '\\' )
		    *--op = ch;
		else {
		    parse_fname (path, subdir, fname, bufsize);
		    goto quit;
		}
	    }
	}
 ...
 */
/* path  : input filename		*/
/* dname : receives directory name	*/
/* fname : receives file name		*/
void parse_fname ( const char *path, char *dname, char *fname, size_t bufsize )
{
	const char *ip;
	const char *delim;
	char *op, *maxop;
	...
	for ( ip=path, op=fname ; op < maxop && (*ip) ; op++, ip++ ) {
	    *op = *ip;
	    if (*ip == '$' || *ip == '/') {
		if ( path < ip && *(ip-1) == '\\' && fname < op )
		    *(--op) = *ip;
		else
		    delim = ip;
	    }
	...
	for ( ip=path, op=dname ; ip <= delim ; op++, ip++ )
	    *op = *ip;
	*op = EOS;

	maxop = fname + bufsize -1;
	for ( op=fname ; op < maxop && (*ip) ; op++, ip++ )
	    *op = *ip;
	if ( op <= maxop ) *op = EOS;
}
...
/* fname  : mkpkgfile name		*/
struct context *push_context ( struct context *cx, const char *module, 
			       const char *newdir, const char *fname )
{
	struct context *ncx;
	...
	*ncx = *cx;		/* copy old struct to new */
	...
	if (module[0])
		...
		maxop = ncx->library + SZ_PATHNAME+1 -1;
		for ( ip=module+1, op=ncx->library ; op < maxop && (*ip) ; op++, ip++ ) {
		    if (*ip == ')') break;
		    *op = *ip;
		}
		*op = EOS;
		...
		safe_strcpy (ncx->library, SZ_PATHNAME+1, module);
	...
	if (newdir[0] && strcmp(newdir,".") != 0 && strcmp(newdir,"./") != 0) {
		...
		os_fpathname ("", ncx->dirpath, SZ_PATHNAME+1);
		ncx->level++;
	    }
	...
	if (fname[0])
	    safe_strcpy (ncx->mkpkgfile, SZ_FNAME+1, fname);
 ...
 */
/* cx : current context	*/
struct context *pop_context ( struct context *cx )
{
	struct context *pcx;
	int root_modlist, level;
 ...
 */
/* cx       : current library context		*/
/* module   : module list is for		*/
/* dflist   : receives filename pointers	*/
/* maxfiles : maxfiles out			*/
void get_dependency_list ( struct context *cx, const char *module, 
			   const char *dflist[], int maxfiles )
{
	char	fname[SZ_FNAME+1];
	int	token, nfiles=0;
	char	*save_cp;
	...
	while ((token = gettok (cx, fname, SZ_FNAME+1)) != 0) {
	    switch (token) {
	    case TOK_NEWLINE:
		goto done;
	    case TOK_FNAME:
 ...
 */
/* cx : current library context		*/
/* module : module to compare dates for	*/
/* lname  : local name of module	*/
/* dflist : list of dependent files	*/
/* useobj : obj exists and is usable	*/
static int up_to_date ( struct context *cx, const char *module, 
			const char *lname, const char *dflist[], int *useobj )
{
 ...
 * a "soft" open.
 */
static int open_mkpkgfile ( struct context *cx )
{
	const char *fname = cx->mkpkgfile;
	struct context *ax;
 ...
 * the file.
 */
static void close_mkpkgfile ( struct context *cx )
{
	if (cx->fp != stdin)
 ...
 */
/* head_cx   : head of context list	*/
/* mkpkgfile : file to search for	*/
/* level     : subdirectory level	*/
static struct context *find_mkpkgfile ( struct context *head_cx, 
					const char *mkpkgfile, int level )
{
	struct context *cx;
 ...
 * while searching.
 */
static int search_mkpkgfile ( struct context *cx )
{
	 ...
	 */
	if (cx->library[0]) {
	    exit_status = do_goto (cx, cx->library);
	    return exit_status;
	}
	 ...
	 */
	while ((tok = gettok (cx, curr, SZ_FNAME+1)) != TOK_BEGIN) {
		 ...
		 * in a subdirectory.
		 */
		parse_modname (curr, module, subdir, fname, SZ_FNAME+1);
		if ((ncx = push_context (cx, module,subdir,fname)) == NULL)
		    exit_status = ERR;
		else {
	...
	safe_strcpy (cx->library, SZ_PATHNAME+1, prev);	/* return module name */
	exit_status = OK;
	return (OK);
}
