...
int sf_scanlist ( struct context *cx )
{
	...
	if (gettok (cx, token, SZ_CMD+1) != TOK_FNAME) {
	    warns ("missing directory name in special file list", "");
	    goto err;
	} else
	    os_fpathname (token, dirname, SZ_PATHNAME+1);
	...
	while ((tok = gettok (cx, token, SZ_CMD+1)) != TOK_BEGIN)
	 ...
	 */
	while (!eol && (tok = gettok (cx, token, SZ_CMD+1)) != TOK_END) {
	    if (tok == EOF || tok == TOK_END)
		break;
		...
		safe_strcpy (stname, SZ_PATHNAME+1, token);

	    /* Get the special file name. */
	    if ((tok = gettok (cx, sfname, SZ_PATHNAME+1)) == TOK_END)
		eol++;
	    if (tok != TOK_FNAME)
		goto badline;

	    /* Get the mkobj command string, if any. */
	    if ((tok = gettok (cx, token, SZ_CMD+1)) == TOK_NEWLINE) {
		mkobj[0] = EOS;
	    } else if (tok == TOK_END) {
		mkobj[0] = EOS;
		eol++;
		 ...
		 * references therein.
		 */
		maxop = mkobj + SZ_CMD+SZ_PATHNAME+1 -1;
		for ( ip=token, op=mkobj ; op < maxop && (*ip) ; op++, ip++ ) {
		    if (*ip == '&') {
			for ( tp=stname ; op < maxop && (*tp) ; op++, tp++ )
			    *op = *tp;
			--op;
		    }
		    else {
...
badline:
	    ...
	    while (!eol && (tok = gettok (cx, token, SZ_CMD+1)) != TOK_NEWLINE)
		if (tok == TOK_END)
		    break;
		else if (tok == EOF)
		    goto err;
 ...
 */
/* dirname : host pathname of directory */
struct sfile *sf_dirsearch ( const char *dirname )
{
	int i;
 ...
 * exact match.
 */
struct sfile *sf_filesearch ( struct sfile *sflist, const char *stname )
/* sflist : special file list	*/
/* stname : standard file name	*/
{
	struct sfile *sfp;
	const char *p1, *p2;
 ...
 * string buffer.
 */
/* cp : lop off everything here and above */
void sf_prune ( char *cp )
{
	struct sfile *sfp, *sf_top;
	int i;
