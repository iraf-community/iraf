...
static int setfuid;		/* Restore file user id			*/
...
static char tapeblock[SZ_TAPEBUFFER+1];
static char *nextblock;
static int nblocks;

static char *getblock ( int );
static int getheader ( int, struct fheader * );
static void printheader ( FILE *, struct fheader *, int );
static void skipfile ( int, struct fheader * );
static int matchfile ( const char *, char *const [] );
static int filetype ( int, struct fheader * );
static int newfile ( const char *, int, int, int, int );
static void copyfile ( int, int, struct fheader *, int	);
static int cchksum ( const char *, int );
static int checkdir ( const char *, int, int, int );
static void strip_blanks ( int, int, long );
 ...
 */
/* fname : filename to be compared to list	*/
/* files : pointer to array of fname pointers	*/
static int matchfile ( const char *fname, char *const files[] )
{
	const char *fn, *ln;
	int firstchar;
 ...
 */
/* in : input file			*/
/* fh : decoded file header (output)	*/
static int getheader ( int in, struct fheader *fh )
{
	char *ip, *op;
	union hblock *hb;
	int n, tape_checksum, ntrys;
	...
	for (ntrys=0;  ;  ntrys++) {
	     ...
	     */
	    if (*hb->dbuf.chksum == '\0' && cchksum ((const char *)hb, TBLOCK) == 0)
		return (EOF);
	    else
		sscanf (hb->dbuf.chksum, "%o", &tape_checksum);

	    for ( ip=hb->dbuf.chksum, n=8 ; n > 0 ; n-- )
		*ip++ = ' ';
	    if (cchksum ((const char *)hb, TBLOCK) != tape_checksum) {
		...
		if (ntrys == 0) {
			...
			cchksum ((const char *)hb, TBLOCK), tape_checksum);
 ...
 */
static int cchksum ( const char *p, int nbytes )
{
	int sum;
...
/* verbose : long format output		*/
static void printheader ( FILE *out, struct fheader *fh, int verbose )
{
	struct _modebits *mp;
	const char *tp;
 ...
 */
/* fname    : pathname of file		*/
/* mode     : file mode bits		*/
/* uid, gid : file owner, group codes	*/
/* type     : text, binary, directory	*/
static int newfile ( const char *fname, int mode, int uid, int gid, int type )
{
	int fd;
	char *cp, *fname_p;
	char fname_tmp[SZ_PATHNAME+1];

	fname_p = fname_tmp;
	strncpy(fname_p, fname, SZ_PATHNAME+1);
	fname_p[SZ_PATHNAME+1-1] = '\0';

	if (len_pathprefix && strncmp(fname_p,pathprefix,len_pathprefix) == 0)
	    fname_p += len_pathprefix;

	if (debug)
	    fprintf (stderr, "newfile `%s':\n", fname_p);

	if (checkdir (fname_p, mode, uid, gid) == ERR)
	    return (ERR);
	...
	if (type == DIRECTORY_FILE) {
		...
		*cp = EOS;
	    ...
	    if (replace)
		os_delete (fname_p);
	    fd = os_createfile (fname_p, mode, type);
	}
 ...
 */
static int checkdir ( const char *path, int mode, int uid, int gid )
{
	char *cp;
	char path_tmp[SZ_PATHNAME+1];

	strncpy(path_tmp, path, SZ_PATHNAME+1);
	path_tmp[SZ_PATHNAME+1-1] = '\0';
	...
	*cp = EOS;
	if (os_access (path_tmp, 0, DIRECTORY_FILE) == YES) {
	    *cp = '/';
	    return (OK);
	}
	*cp = '/';
	...
	for (cp=path_tmp;  *cp;  cp++) {
	    ...
	    *cp = EOS;
	    if (os_access (path_tmp, 0, DIRECTORY_FILE) == NO) {
		if (os_createdir (path_tmp, RWXR_XR_X) == ERR) {
		    fprintf(stderr,"cannot create directory `%s'\n",path_tmp);
		    *cp = '/';
		    return (ERR);
		} else
		    os_setowner (path_tmp, uid, gid);
	    }
	    *cp = '/';
	}
