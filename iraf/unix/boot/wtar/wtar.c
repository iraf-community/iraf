...
static int debug=NO;		/* Print debugging messages		*/
static int omitbinary;		/* omit binary files			*/
...
static void putfiles ( const char *, int, const char * );
static void tarfileout ( const char *, int, int, const char * );
static void endtar ( int );
static int u_fmode ( int, int );
static int putheader ( struct fheader *, int );
static void copyfile ( const char *, struct fheader *, int, int );
static void printheader ( FILE *, struct fheader *, int );
static int cchksum ( const char *, int );
static int putblock ( int, const char * );
...
{
	static char dotstr[2] = {'.','\0'};
	static char *def_flist[2] = { dotstr, NULL };
	char **flist;
	char *argp;
	int argno, ftype, i;
...
/* dir  : directory name		*/
/* out  : output file			*/
/* path : pathname of curr. directory	*/
static void putfiles ( const char *dir, int out, const char *path )
{
	...
	snprintf (newpath, SZ_PATHNAME+1, "%s%s", dname(path), dir);
	safe_strcpy (newpath, SZ_PATHNAME+1, dname(newpath));
 ...
 */
/* fname : file to be output	*/
/* out   : output stream	*/
/* ftype : file type		*/
/* path  : current path		*/
static void tarfileout ( const char *fname, int out, int ftype, 
			 const char *path )
{
	struct _finfo fi;
	struct fheader fh;
	 ...
	 */
	ZFINFO ((PKCHAR *)vfn2osfn(fname,0), (XLONG *)&fi, &status);
	if (status == XERR) {
	    fflush (stdout);
	...
	safe_strcpy (fh.name, NAMSIZ, path);
	safe_strcat (fh.name, NAMSIZ, fname);
	strcpy (fh.linkname, "");
	fh.linkflag = 0;

	if (ftype == DIRECTORY_FILE) {
	    safe_strcpy (fh.name, NAMSIZ, dname(fh.name));
	    fh.size  = 0;
	    fh.isdir = 1;
	    fh.linkflag = LF_DIR;
	} else {
 ...
 */
/* fh  : (input) file header	*/
/* out : output file descriptor	*/
static int putheader ( struct fheader *fh, int out )
{
	const char *ip;
	int n;
	union hblock hb;
...
static void printheader ( FILE *fp, struct fheader *fh, int verbose )
{
	struct _modebits *mp;
	const char *tp;
 ...
 */
/* fname : file being read from		*/
/* fh    : file header structure	*/
/* ftype : file type, text or binary	*/
/* out   : output file			*/
static void copyfile ( const char *fname, struct fheader *fh, 
		       int ftype, int out )
{
	char *bp;
	int i;
 ...
 */
static int putblock ( int out, const char *buf )
{
	int nbytes = 0;
 ...
 * in a single /.
 */
static const char *dname ( const char *dir )
{
	...
	for ( ip=dir, op=path ; op < maxop && (*ip) ; op++, ip++ ) {
	    while (*ip == '/' && *(ip+1) == '/')
		ip++;
	    *op = *ip;
	}
