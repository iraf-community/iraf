...
static int * ggreed = lkst[0].lset;
static int * pgo = wsets[0].ws.lset;
static int *yypgo = &nontrst[0].tvalue;

static int maxspr = 0;  /* maximum spread of any entry */
static int maxoff = 0;  /* maximum offset into a array */
static int *pmem = mem;
static int *maxa;
...
static void arout ( const char *, int *, int );
...
void callopt( void )
{
	...
	for ( ; ; ) {
	    ...
	    case '\n':
		if ( NSTATES <= nstate + 1 ) error("too many states");
...
static void gin( int i )
{
	...
	for ( p=a; p < &a[ACTSIZE]; ++p ) {
		...
		if ( maxa < s ) {
		    maxa=s;
		    if ( a + ACTSIZE <= s ) error( "a array overflow" );
		}
		if ( *s ) goto nextgp;
	    }
	    ...
	    if ( maxa < p ) {
		maxa=p;
		if ( a + ACTSIZE <= p ) error( "a array overflow" );
	    }
	    *p = *q2;
	    for ( r=q1 ; r<q2 ; r+=2 ) {
		s = p + *r + 1;
...
{	/* write summary */
	...
	fprintf( foutput, "Optimizer space used: input %ld/%ld, output %ld/%ld\n",
		 (long)(pmem-mem+1), (long)MEMSIZE, (long)(maxa-a+1), (long)ACTSIZE );
...
static void aoutput( void )
{
	...
	fprintf (fsppout, "define\tYYLAST\t\t%ld\n", (long)(maxa-a+1));
...
static void arout ( const char *s, int *v, int n )
{
	int i, j;
...
static int gtnm( void )
{
	...
	if ( mem + MEMSIZE <= pmem ) error( "out of space" );
	*pmem++ = s*val;
	return ( c );
