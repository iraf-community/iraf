...
static struct looksets clset;	/* temporary storage for lookahead computations */
...
static int tstates[NTERMS];	/* states generated by terminal gotos */
static int ntstates[NNONTERM];	/* states generated by nonterminal gotos */
static int mstates[NSTATES];	/* chain of overflows of term/nonterm generation lists  */
...
static struct looksets *pfirst[NNONTERM+2];
static int pempty[NNONTERM+1];	/* vector of nonterminals nontrivially deriving e */
...
static struct looksets *flset( struct looksets * );
...
{	/* put out other arrays, copy the parsers */
	...
	/* Copy parser text, expand the $A and $B macros.  The $A macro is
	 * repaced by the parser tables (ftable), and the $B macro by the
	 * user actions.
	 */
...
{	/* creates output string for item pointed to by pp */
	...
	for ( p=pp ; *p>0 ; ++p ) ;
	p = prdptr[-*p];
	snprintf( sarr, ISIZE, "%s%s", nontrst[*p-NTBASE].name, " : " );

	for ( ; ; ) {
	    const char *dp;
	    if ( ++p==pp ) dp="_";
	    else dp=" ";
	    snprintf( sarr + strlen(sarr), ISIZE - strlen(sarr), "%s", dp );
	    if ((i = *p) <= 0) break;
	    snprintf( sarr + strlen(sarr), ISIZE - strlen(sarr), "%s", symnam(i) );
	    if ( ISIZE-30 < strlen(sarr) ) error( "item too big" );
	}

	if ( (i = *pp) < 0 ) { /* an item calling for a reduction */
	    snprintf( sarr + strlen(sarr), ISIZE - strlen(sarr), "%s%d)", "    (", -i );
	}
...
static struct wset *zzcwp = wsets;
static int *zzmemsz = mem0;
...
{	/* output the summary on the tty */
	...
	if ( foutput != NULL ) {
	    ...
	    fprintf( foutput, "%ld/%ld working sets used\n", (long)(zzcwp-wsets),  (long)WSETSIZE );
	    fprintf( foutput, "memory: states,etc. %ld/%ld, parser %ld/%ld\n", 
		     (long)(zzmemsz-mem0), (long)MEMSIZE,
		     (long)(memp-amem), (long)ACTSIZE );
...
/* VARARGS1 */
void error( const char *argsformat, ... )
...
static int setunion( int *a, int *b )
{
...
{	/* compute an array with the beginnings of productions yielding 
	...
	if ( pmem != &pyield[nprod] ) {
	    error( "internal Yacc error: pyield %lx",
		   (unsigned long)(pmem-&pyield[nprod]) );
	}
}
...
{	/* sorts last state,and sees if it equals earlier ones. returns state number */
	...
	if ( NSTATES <= nstate ) error("too many states");
	p1 = pstate[nstate];
	p2 = pstate[nstate+1];
...
static int pidebug = 0; /* debugging flag for putitem */
...
static struct looksets *flset( struct looksets *p )
{
