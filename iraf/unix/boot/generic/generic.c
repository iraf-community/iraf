...
static struct _for forstk[SZ_FORSTK];
static int forlev;
static int pass_output = 1;
static int clobber = NO;

static void set_type_string ( char );
static int evaluate_expr ( void );
static int parse_relational ( int *, int *, int * );
static const char *make_typed_filename ( const char *, char );
static int nextch ( void );
static int relop ( void );
static int gch ( void );
static int gsize ( char );
static void uch ( char );
...
int main ( int argc, char *argv[] )
{
	...
	for (p = &argv[1];  *p != NULL;  p++) {
		...
		case 'o':
		    if (*(p+1) != NULL)
			snprintf (genfname, SZ_FNAME+1, "%s", *++p);
		    break;
		case 'p':
		    if (*(p+1) != NULL)
			prefix = *++p;
	...
	for (n=0;  n < nfiles;  n++) {
	    char *op;
	    snprintf (input_file, SZ_FNAME+1, "%s", files[n]);
		...
		*op = EOS;
		...
		if (genfname[0] == EOS) {
		    ...
		    snprintf(template, SZ_FNAME+1, "%s$t", input_file);
			...
			snprintf (template, SZ_FNAME+1, "%s$t", genfname);
		    else 
			snprintf (template, SZ_FNAME+1, "%s", genfname);
		}

		if (genfname[0] == EOS || strlen (types) > 1)
		    tfilename = make_typed_filename (template, *t);
		else
		    tfilename = template;
		 ...
		 */
		if (genfname[0] == EOS) {
		    snprintf (fname, SZ_FNAME, "%s%s%s%s", 
			      prefix, tfilename, ".", extension);
		}
		else {
		    snprintf (fname, SZ_FNAME, "%s%s", prefix, tfilename);
		}
 ...
 */
static const char *make_typed_filename ( const char *template, char type_char )
{
	...
	maxop = fname + SZ_FNAME+1 -1;
	for ( ip=template, op=fname ; op < maxop && *ip != EOS ; op++ ) {
	    if (*ip == '$' && *(ip+1) == '$') {
		*op = '$';
		ip += 2;
	    } else if (*ip == '$' && *(ip+1) == 't') {
 ...
 */
static void set_type_string ( char ch )
{
	const char *ip;
	char *op;
 ...
 */
void output_indef ( char ch )	/* output INDEF, INDEFS, INDEFL, etc. */
{
	outstr(yytext);
 ...
 * in upper case.
 */
void output_upper ( const char *s )
{
	const char *p;
	int ch;
...
static char expr_buf[80], *expr;
static int level = 0;

static struct if_stack {
	int	oldstate;
	int	active;
} stk[MAXNEST];
...
void do_if( void )
{
	...
	level += 1;
	if ( MAXNEST <= level ) {
	    fprintf (stderr, "level exceeds buffer size\n");
	    exit (OSOK+1);
	}
	p = &stk[level];
 ...
 */
void do_endif( void )		/* $endif statement */
{
	int	ch;
...
static int parse_relational ( int *size1, int *size2, int *op )
{
	expr += 6;				/* ... (t1) */
