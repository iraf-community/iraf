 ...
 */
#define SZ_OBUF		(1024)
#define DECL_OBUF	char *op; char *np; char obuf[SZ_OBUF+1];
...
static int imp_left;
static int imp_bottom;
static int imp_width;
static int imp_height;
static int imp_penbase  = DEF_PENBASE;
static int imp_penslope = DEF_PENSLOPE;
	...
	for (argno=1;  (argp = argv[argno]) != NULL;  argno++) {
		...
		case 'p':
			...
			if (argv[argno+1] == NULL) {
			    ...
			    strncpy (penparam, argv[++argno], SZ_PENPARAM);
			    penparam[SZ_PENPARAM-1] = '\0';
			}
		    else {
			strncpy (penparam, argv[argno]+2, SZ_PENPARAM);
			penparam[SZ_PENPARAM-1] = '\0';
		    }
