...
int ZOPNND ( PKCHAR *pk_osfn, XINT *mode, XINT *chan )
{
	...
	snprintf (osfn, SZ_NAME*2, (const char *)pk_osfn, getuid(), getuid());
	...
	if (strncmp (osfn, "inet:", 5) == 0) {
		...
		memmove ((char *)&host_addr, hp->h_addr, sizeof(host_addr));
	    } else
		goto err;
	...
	case READ_WRITE:
		...
		memmove ((char *)&sockaddr.sin_addr, (const char *)&host_addr,
			 sizeof(host_addr));
		...
		safe_strcpy (sockaddr.sun_path,
			     sizeof(sockaddr.sun_path), np->path1);
	...
	case NEW_FILE:
		...
		memset ((char *)&sockaddr, 0, sizeof(sockaddr));
		sockaddr.sun_family = AF_UNIX;
		safe_strcpy (sockaddr.sun_path, sizeof(sockaddr.sun_path), np->path1);
...
int ZAWRND ( XINT *chan, XCHAR *buf, XSIZE_T *nbytes, XLONG *offset )
{
	...
	for (nwritten=0;  nwritten < *nbytes;  nwritten += n, ip+=n) {
	    ...
	    if (np->flags & F_TEXT) {
		const XCHAR *ipp = (XCHAR *)ip;
		char *op = (char *)obuf;
		char *maxop = (char *)obuf + (n / sizeof(XCHAR)) -1;
