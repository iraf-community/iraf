...
static struct lprinter dpr;		/* device table			*/
static struct oprinter lpr;		/* printer descriptor		*/
static int lpr_inuse = NO;		/* set if printer is open	*/
static char lpstr[SZ_LPSTR+1];		/* save zopnlp argument		*/
 ...
 */
/* printer : logical name of printer device	*/
...
int ZOPNLP ( PKCHAR *printer, XINT *mode, XINT *chan )
{
	...
	*ip++ = EOS;
	...
	lpr.wbytes = 0L;
	lpr.lp = &dpr;
	safe_strcpy (lpr.spoolfile, SZ_PATHNAME+1, dpr.spoolfile);
...
int ZCLSLP ( XINT *chan, XINT *status )
{
	...
	if (*(lpr.lp->dispose) != EOS) {
		...
		op = (char *)cmd;
		for ( ip=lpr.lp->dispose ; op < maxop && *ip != EOS ; op++ ) {
		    *op = *ip;
		    ip++;
		    if (*op == '$' && *ip == 'F') {
			for ( f=lpr.spoolfile ; op < maxop && *f != EOS ; op++, f++ )
			    *op = *f;
