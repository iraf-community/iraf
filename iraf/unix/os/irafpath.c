...
const char *irafpath ( const char *fname )
{
...
#ifdef LINUXPPC
	safe_strcat (pathname, SZ_PATHNAME+1, "linuxppc");
#else
#ifdef CYGWIN
	safe_strcat (pathname, SZ_PATHNAME+1, "cygwin");
#else
#ifdef REDHAT
	safe_strcat (pathname, SZ_PATHNAME+1, "redhat");
#else
#ifdef LINUX
#ifdef X86_64
	safe_strcat (pathname, SZ_PATHNAME+1, "x86_64-linux-generic");
#else
	safe_strcat (pathname, SZ_PATHNAME+1, "linux");
#endif
#else
#ifdef BSD
	safe_strcat (pathname, SZ_PATHNAME+1, "freebsd");
#else
#ifdef MACOSX
	...
	{ 
	    const char *irafpath;
		    ...
		    safe_strcat (pathname, SZ_PATHNAME+1, "macosx");
                else if (strcmp (irafarch, "macintel") == 0) 
		    safe_strcat (pathname, SZ_PATHNAME+1, "macintel");
                else 
		    safe_strcat (pathname, SZ_PATHNAME+1, "macosx");
            }
        }
...
#ifdef SOLARIS
#ifdef X86
	safe_strcat (pathname, SZ_PATHNAME+1, "sunos");
#else
	safe_strcat (pathname, SZ_PATHNAME+1, "ssol");
#endif
#else
#ifdef sparc
	safe_strcat (pathname, SZ_PATHNAME+1, "sparc");
#else
#endif
#endif
#endif
