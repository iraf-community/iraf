...
int ZOPDPR ( PKCHAR *osfn, PKCHAR *bkgfile, PKCHAR *queue, XINT *jobcode )
{
	...
	if (access ((const char *)osfn, 1) == ERR) {
	    *jobcode = XERR;
	    return XERR;
	} else if (access ((const char *)bkgfile, 4) == ERR) {
	    *jobcode = XERR;
	    return XERR;
	}
	...
	for (ip=(const char *)queue;  isspace (*ip);  ip++)
	    ;
	if (*ip != EOS) {
	    if (*ip == '+' || *ip == '-') {
		delta = 1;
	...
	if (pid == 0) {
	     ...
	     * the file upon exit.
	     */
	    execl ((const char *)osfn, (const char *)osfn, "-d", (char *)bkgfile, 0);
