...
static int debug_ks = 0;		/* print debug info on stderr	  */
static char debug_file[64] = {'\0'};	/* debug output file if nonnull   */
static FILE *debug_fp = NULL;		/* debugging output		  */

static	jmp_buf jmpbuf;
static	int jmpset = 0;
static	int recursion = 0;
static	int parent = -1;
static	signal_handler_t old_sigcld;
static	int ks_pchan[MAXOFILES];
static	int ks_achan[MAXOFILES];

static	int ks_getresvport( int * );
static	int ks_rexecport( void );
static	int ks_socket( const char *, u_long, int, const char * );
static	int ks_geti( int );
static	int ks_puti( int, int );
static	int ks_getlogin( const char *, char *, char *, struct ksparam * );
static	void dbgmsg ( const char *argsformat, ... );
static	char *ks_getpass( const char *, const char * );
static	void ks_onsig( int /*, int *, int * */ );
static	void ks_reaper( int /*, int *, int * */ );
 ...
 */
/* x_server : type of connection */
/* mode     : access mode (not used) */
/* chan     : receives channel code (socket) */
int ZOPNKS ( PKCHAR *x_server, XINT *mode, XINT *chan )
{
	const char *ip;
	char *op, *maxop;
	const char *server = (const char *)x_server;
	...
	host[0] = username[0] = password[0] = 0;
	...
	if (strncmp (ip, "-log", 4) == 0) {
		...
		;
	    maxop = debug_file + 64 -1;
	    for ( op=debug_file ; *ip && !isspace(*ip) ; ip++ ) {
		if ( op < maxop ) {
		    *op = *ip;
		    *op++;
		}
	...
	dbgmsg ("---------------------------------------------------------\n");
	dbgmsg ("zopnks (`%s')\n", server);
	dbgmsg ("kstype=%d, prot=%d, host=`%s', cmd=`%s')\n",
	...
	if (proctype == IRAFKS_DIRECT) {
		...
		*chan = s;
		...
		if (np == NULL) {
		    ...
		    ip = np;
		dbgmsg ("S:detached in.irafksd, port=%d, auth=%d, timeout=%d\n",
		    port, auth, nsec);
		...
		if ((nsec = ks_geti(0)) < 0)
		    { status = 3; goto d_err; }
		dbgmsg ("S:client spawned in.irafksd, port=%d, timeout=%d\n",
		    port, nsec);
	    }
	    ...
	    dbgmsg ("S:fork in.irafksd, port=%d, timeout=%d\n", port, nsec);
	    pid = fork();
	    if (pid < 0) {
		status = 4;
		goto d_err;
	    ...
	    if (pid) {
d_err:		dbgmsg ("S:in.irafksd parent exit, status=%d\n", status);
		if (!detached) {
		    ks_puti (1, status);
		    ks_puti (1, port);
		}
	    ...
	    dbgmsg ("S:in.irafksd started, pid=%ld ppid=%ld pgid=%ld\n",
		...
		dbgmsg ("S:in.irafksd: client port = %d\n", s_port);
...
s_err:		    dbgmsg ("S:in.irafksd fork complete, status=%d\n",
		    ...
		    u_long  n_addr, addr;
		    const unsigned char *ap = (const unsigned char *)&n_addr;

		    dbgmsg ("S:irafks server started, pid=%ld ppid=%ld pgid=%ld\n",
			(long)getpid(), (long)getppid(), (long)getpgrp());
		    ...
		    */
		    ...
		    addr = ntohl(n_addr);
		    snprintf (obuf, SZ_LINE, "%d.%d.%d.%d", ap[0],ap[1],ap[2],ap[3]);
			...
			dbgmsg ("S:irafks connected on port %d\n", s_port);
	...
	if (ks_getlogin (host, username, password, &ks) == ERR) {
	    ...
	    *chan = rexec (&hostp, ks_rexecport(), username, password, cmd, 0);
		...
		goto r_err;
	    localhost[SZ_FNAME-1]='\0';
	    if ((hp = gethostbyname (localhost)) == NULL)
		goto r_err;
	    snprintf (callback_cmd, SZ_LINE, "%s callback %d@%s",
		cmd, s_port, hp->h_name);
	    dbgmsg ("rexec to host %s: %s\n", host, callback_cmd);
...
r_err:		dbgmsg ("rexec-callback connect failed\n");
		...
		*chan = ERR;
		...
		dbgmsg ("connected to irafks server on fd=%d\n", fd);
		*chan = fd;
	    ...
	    dbgmsg ("C:connect to in.irafksd host=%s client port=%d\n",
		host, s_port);
...
again:
		...
		if (pid) {
		    ...
		    close (pout[0]);
retry:
		    dbgmsg ("C:send port=%d, timeout=%d to irafks.e\n",
			ks.port, ks.timeout);
		    ...
		    port = ks_geti (pin[0]);
		    dbgmsg ("C:irafks.e returns port=%d\n", port);
			...
			snprintf (command, SZ_LINE, "%s in.irafksd", cmd);
	     ...
	     */
	    dbgmsg ("C:request irafks server for client port %d\n", s_port);
	    ...
	    /* Wait for the server to call us back. */
	    if ((tfd = accept (s, (struct sockaddr *)0, (int *)0)) < 0) {
c_err:		dbgmsg ("C:zfioks client status=%o\n", status);
		close(t);  close(s);
		kill (pid, SIGTERM);
		*chan = ERR;
	    } else {
		close(t);  close(s);  fd = dup(tfd);  close(tfd);
		dbgmsg ("C:connected to irafks server on fd=%d\n", fd);
		*chan = fd;
	    }
	}
	...
	dbgmsg ("zopnks returns status=%ld\n", (long)(*chan));

	if ( *chan == ERR ) return XERR;
	else return XOK;
}
 ...
 */
/* chan   : socket to kernel server	*/
/* status : receives close status	*/
int ZCLSKS ( XINT *chan, XINT *status )
{
	...
	*status = close (*chan);
	...
	dbgmsg ("server [%ld] terminated, status = %ld\n", (long)(*chan), (long)(*status));
 ...
 */
/* chan     : kernel server channel (socket)	*/
/* buf      : output buffer			*/
...
int ZARDKS ( XINT *chan, XCHAR *buf, XSIZE_T *totbytes, XLONG *loffset )
{
	...
	if (debug_ks > 1)
	    dbgmsg ("read %d bytes from KS channel %d:\n", op-(char *)buf, fd);
...
int ZAWRKS ( XINT *chan, XCHAR *buf, XSIZE_T *totbytes, XLONG *loffset )
{
	...
	zfd[fd].nbytes = nbytes = *totbytes;
	if (debug_ks > 1)
	    dbgmsg ("initiate write of %d bytes to KS channel %d\n",
		nbytes, ofd);
	...
	jmpset++;
	if (setjmp (jmpbuf) == 0)
	    zfd[fd].nbytes = write (ofd, (const char *)buf, nbytes);
	else
	    zfd[fd].nbytes = ERR;
	...
	if (debug_ks > 1)
	    dbgmsg ("wrote %d bytes to KS channel %d:\n", zfd[fd].nbytes, ofd);
...
static void ks_reaper ( int sig /*, int *arg1, int *arg2 */ )
{
	...
	pid_t pid=0;
	    ...
	    dbgmsg ("ks_reaper -- pid=%ld, status=%d\n", (long)pid, status);
 ...
 */
/* chan : not used; all KS channels have same status */
 ...
 */
static int ks_socket ( const char *host, u_long addr, int port, 
		       const char *mode )
{
	struct	sockaddr_in sockaddr;
	struct	hostent *hp;
	...
	if (addr) {
		...
		goto failed;
	    memmove((char *)&sockaddr.sin_addr, (const char *)hp->h_addr, hp->h_length);
	} else
...
static int ks_getresvport ( int *alport )
{
	...
	for (;;) {
	    ...
	    dbgmsg ("ks_getresvport: decr errno=%d (%s) alport=%d -> %d\n", 
 ...
 */
static void dbgsp ( pid_t pid )
{
	long i, nsp = ((parent > 0) ? (pid - parent) : 0);
	for (i=0; i < nsp; i++)
	    fprintf (debug_fp, "  ");
}
...
static void dbgmsg ( const char *argsformat, ... )
{
	...
	pid_t pid;
	if (debug_ks) {
	    fprintf( debug_fp, "[%5d] ", (pid = getpid()) );
	    dbgsp( pid );
	    va_start( ap, argsformat );
	    vfprintf( debug_fp, argsformat, ap );
	    va_end(ap);
	    fflush (debug_fp);
	}
}
...
static	int ks_getword ( const char **, char * );
static	char *ks_username ( const char *, char *, char * );
static	char *ks_sysname ( const char *, char * );
static	struct irafhosts *ks_rhosts ( const char * );
static	void ks_whosts ( struct irafhosts *, const char * );
 ...
 */
/* hostname  : node we wish a login for		*/
/* loginname : receives the login name		*/
/* password  : receives the login password	*/
/* ks        : networking parameters		*/
static int ks_getlogin ( const char *hostname, 
			 char *loginname, char *password, struct ksparam *ks )
{
	char userfile[SZ_PATHNAME];
	char sysfile[SZ_PATHNAME];
	char fname[SZ_PATHNAME];
	char username[SZ_NAME];
	const char *namep, *authp;
	struct irafhosts *hp;
	struct nodelist *np;
	int update = 0;
	int auth;
	...
	if (hp = ks_rhosts (userfile)) {
		...
		safe_strcpy (fname, SZ_PATHNAME, username);
		safe_strcat (fname, SZ_PATHNAME, ".OLD");
		unlink (fname);
	...
	if (np->login[0] && strcmp(np->login,"disable") == 0) {
	    ...
	    safe_strcpy (loginname, SZ_NAME+1, np->login);
	} else
	    safe_strcpy (loginname, SZ_NAME+1, username);
...
query:		if (namep = ks_getpass (loginname, hostname))
		    safe_strcpy (password, SZ_NAME+1, namep);
		else
		    password[0] = EOS;
	    } else
		safe_strcpy (password, SZ_NAME+1, np->password);
	} else
	    password[0] = EOS;
 ...
 */
static char *ks_username ( const char *filename, 
			   char *pathname, char *username )
{
	struct passwd *pwd;
	...
	safe_strcpy (pathname, SZ_PATHNAME, pwd->pw_dir);
	safe_strcat (pathname, SZ_PATHNAME, "/");
	safe_strcat (pathname, SZ_PATHNAME, IRAFHOSTS);
	if (username)
	    safe_strcpy (username, SZ_NAME, pwd->pw_name);
 ...
 * function value.
 */
static char *ks_sysname ( const char *filename, char *pathname )
{
	PKCHAR	irafdir[SZ_PATHNAME+1];
	...
	safe_strcpy (pathname, SZ_PATHNAME, (const char *)irafdir);
	safe_strcat (pathname, SZ_PATHNAME, filename);
 ...
 */
static struct irafhosts *ks_rhosts ( const char *filename )
{
	struct irafhosts *hp;
	struct nodelist *np;
	struct stat st;
	...
	/* Get file header. */
	op_count=0;
	while (fgets (op, SZ_LINE-op_count, fp)) {
	    if (op[0] == '#' || isspace(op[0])) {
		if ( MAX_HEADERLINES <= hp->nheaderlines ) {
		    fprintf(stderr,"[Warning] hp->nheaderlines exceeds buffer size\n");
		    hp->nheaderlines--;
		}
		hp->header[hp->nheaderlines++] = op;
		op += strlen(op) + 1;
		op_count += strlen(op) + 1;
	    } else {
		safe_strcpy (lbuf, SZ_LINE, op);
		break;
	    }
	    if ( SZ_LINE-op_count <= 0 ) {
		safe_strcpy (lbuf, SZ_LINE, op);
		fprintf(stderr,"[Warning] buffer hp->sbuf is full\n");
		break;
	    }
	}
	...
	do {
		...
		/* Node entry.
		 */
		if ( MAX_NODES <= hp->nnodes ) {
		    fprintf(stderr,"[Warning] hp->nnode exceeds buffer size\n");
		    hp->nnodes--;
		}
		np = &hp->node[hp->nnodes++];	/* nodename */
		if ( 0 < SZ_LINE-op_count ) {
		    safe_strcpy (op, SZ_LINE-op_count, word);
		    np->name = op;
		    op += strlen(op) + 1;
		    op_count += strlen(op) + 1;
		}
 ...
 */
static int ks_getword ( const char **ipp, char *obuf )
{
	const char *ip = *ipp;
	char *op = obuf;
	char *maxop = obuf + SZ_LINE -1;
 ...
 */
static void ks_whosts ( struct irafhosts *hp, const char *filename )
{
	struct nodelist *np;
	const char *ip;
	int fd, q, i;
 ...
 * password.
 */
static char *ks_getpass ( const char *user, const char *host )
{
