 ...
 *   group and world.
 *
 * NOTE: THIS TASK MUST BE INSTALLED IN HBIN WITH OWNER=ROOT AND
...
static struct file {			/* special files for "device"	*/
	char f_name[SZ_FNAME];		/* file name			*/
	struct stat f_sbuf;		/* stat buffer			*/
} sfiles[NSFILES];

static int debug=0;
static int nsfiles;			/* number of special files	*/
/* static int mode; */			/* 07 mode, ie, 04, 02, or 06	*/

static int alloc ( char *const [], int );
static int dealloc ( char *const [] );
static int findsfs ( char *const [] );
...
int main ( int argc, char *argv[] )
{
	...
	if (geteuid()) {
	    ...
	    fprintf (stderr, "(rerun installer, or)\n");
 ...
 * logged in.
 */
/* statonly : if set, just return device status */
static int alloc ( char *const argv[], int statonly )
{
	struct file *fp;
	int ruid, mode;
	int rgid, i;
 ...
 * sfiles.
 */
static int dealloc ( char *const argv[] )
{
	struct file *fp;
	int uid, ruid, i;
 ...
 */
static int findsfs ( char *const argv[] )
{
	struct file *fp;
	const char *fname;
	const char *argp, *ip;

	for ( nsfiles=0 ; nsfiles < NSFILES && (argp = argv[nsfiles]) ; nsfiles++ ) {
	    fp = &sfiles[nsfiles];
	    ...
	    snprintf (fp->f_name, SZ_FNAME, "/dev/%s", fname);
	    if (stat (fp->f_name, &fp->f_sbuf) == -1) {
		snprintf (fp->f_name, SZ_FNAME, "/dev/rmt/%s", fname);
		if (stat (fp->f_name, &fp->f_sbuf) == -1) {
		    printf ("alloc: cannot fstat %s\n", fp->f_name);
		    continue;
		}
