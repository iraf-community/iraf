 ...
 */
/* osfn            : name of executable file		*/
/* inchan, outchan : IPC channels (parent reads inchan)	*/
int ZOPCPR ( PKCHAR *osfn, XINT *inchan, XINT *outchan, XINT *pid )
{
	 ...
	 */
	if (access ((const char *)osfn, 1) == ERR) {
	    *pid = XERR;
	    return XERR;
	}
...
err:	    close (pin[0]);  close (pin[1]);
	...
	if (*pid == 0) {
	     ...
	     * subprocess.
	     */
	    execl ((const char *)osfn, (const char *)osfn, "-c", 0);
...
int ZARDPR ( XINT *chan, XCHAR *buf, XINT *maxbytes, XLONG *loffset )
{
	...
	if (debug_ipc)
	    fprintf (stderr,
		"[%ld] initiate read for %ld bytes from IPC channel %d\n",
		(long)getpid(), (long)(*maxbytes), fd);
	...
	if (ipc_isatty) {
		...
		if ((pr_ionbytes[fd] = nbytes = read (fd, ibuf, maxch)) > 0) {
		    maxop = buf + nbytes -1;
		    for ( ip=ibuf, op=buf ; op <= maxop ; op++, ip++ )
			*op = *ip;
	...
	if (ipc_in > 0)
	    write (ipc_in, (const char *)&temp, 2);
	...
	if (debug_ipc) {
	    fprintf (stderr, "[%ld] read %ld bytes from IPC channel %d:\n",
		(long)getpid(), (long)(op - (char *)buf), fd);
	    write (2, (const char *)buf, op - (char *)buf);
	}

	if (ipc_in > 0)
	    write (ipc_in, (const char *)buf, op - (char *)buf);
 ...
 * streaming binary file.
 */
/* chan : not used; all IPC channels have same status */
