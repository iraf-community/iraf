...
static struct dplotter dpltr;		/* machdep plotter info		*/
static struct oplotter pltr;		/* open plotter descriptor	*/
static char plstr[SZ_PLSTR+1];		/* save zopnpl argument		*/
static int pltr_inuse = NO;		/* set if plotter is open	*/
 ...
 */
/* plotter : plotter device descriptor		*/
/* mode    : file access mode			*/
/* chan    : UNIX file number (output)		*/
int ZOPNPL ( PKCHAR *plotter, XINT *mode, XINT *chan )
{
	 ...
	 * strings.
	 */
	safe_strcpy (plstr, SZ_PLSTR+1, (const char *)plotter);
	...
	*ip++ = EOS;
	...
	pltr.wbytes = 0L;
	pltr.pl = &dpltr;
	safe_strcpy (pltr.spoolfile, SZ_PATHNAME+1, dpltr.spoolfile);
...
int ZCLSPL ( XINT *chan, XINT *status )
{
	...
	if (*(pltr.pl->dispose) != EOS) {
		 ...
		 */
		maxop = (char *)cmd + sizeof(cmd) -1;
		op = (char *)cmd;
		for ( ip=pltr.pl->dispose ; op < maxop && *ip != EOS ; op++ ) {
		    *op = *ip;
		    ip++;
		    if (*op == '$' && *ip == 'F') {
			for ( f=pltr.spoolfile ; op < maxop && *f != EOS ; op++, f++ )
			    *op = *f;
...
int ZSTTPL ( XINT *chan, XINT *param, XLONG *lvalue )
{
	...
	case FSTT_BLKSIZE:
	    *lvalue = 0L;
	    break;
	default:
	    x_status = ZSTTBF (chan, param, lvalue);
	}
