...
static struct proc *get_processtable ( int, int * );
 ...
 */
/* kmem    : fd of kernel memory file */
/* o_nproc : number of processes in output table */
static struct proc *get_processtable ( int kmem, int *o_nproc )
{
...
int uid_executing ( int uid )
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
	    snprintf (fname, 256, "/proc/%s", direntp->d_name);
	    if (stat (fname, &st))
		return (0);
