...
static	int zmtgetfd ( struct mtdesc * );
static	struct mtdesc *zmtdesc ( const char *, int, const char *, struct _mtpos * );
static	int zmtbsr ( int, int );
static	int zmtbsf ( int, int );
static	int zmtfsr ( int, int );
static	int zmtfsf ( int, int );
...
static	void zmtdbgclose ( struct mtdesc * );
static	void zmtdbg ( struct mtdesc *, const char * );
static	void zmtfls ( struct mtdesc * );
static	void zmtdbgn ( struct mtdesc *, const char *, ... );
 ...
 */
/* device  : device name					*/
/* acmode  : access mode: read_only or write_only for tapes	*/
/* devcap  : tapecap entry for device				*/
	     ...
	     XINT *newfile, XINT *chan )
{
	...
	mp = zmtdesc ((const char *)device, *acmode, 
		      (const char *)devcap, (struct _mtpos *)devpos);
	if (mp == NULL) {
	    *chan = XERR;
	    return XERR;
	}
	...
	*chan = 0;
	...
	zmtdbgn (mp, "tapeused = %ld", (long)(pp->tapeused));
	zmtfls (mp);
	...
	zmtdbgn (mp, "file = %ld%s", (long)(pp->filno), ateot(pp) ? " (EOT)" : "");
	zmtdbgn (mp, "record = %ld", (long)(mp->mtpos.recno));
	zmtfls (mp);
	...
	*o_status = XERR;
	...
	if (mp->acmode == READ_ONLY) {
		...
		if (zmtrew(fd) == ERR)
		    ...
		    pp->filno = pp->recno = 1;
		    zmtdbgn (mp, "file = %ld", (long)(pp->filno));
		    zmtdbgn (mp, "record = %ld", (long)(pp->recno));
		}
	    }
...
int ZZRWMT ( PKCHAR *device, PKCHAR *devcap, XINT *o_status )
{
	...
	/* Open the main device descriptor. */
	mp = zmtdesc ((const char *)device, READ_ONLY, 
		      (const char *)devcap, NULL);
	if (mp == NULL) {
	    *o_status = ERR;
	    return *o_status;
	}
 ...
 */
static int zmtgetfd ( struct mtdesc *mp )
{
	int fd;
 ...
 */
/* dev      : device name or pathname		*/
/* u_acmode : read only or write only for tapes	*/
static int zmtopen ( const char *dev, int u_acmode )
{
	...
	if (dev[0] == '/') {
	    ...
	    pwd = getpwuid (getuid());
	    if (pwd != NULL) {
		safe_strcpy (path, SZ_PATHNAME+1, pwd->pw_dir);
		safe_strcat (path, SZ_PATHNAME+1, &dev[1]);
		endpwent();
	    ...
	    /* System device. */
	    strcpy (path, "/dev/");
	    safe_strcat (path, SZ_PATHNAME+1, dev);
 ...
 */
/* device : host device to be used for i/o	*/
/* acmode : iraf file access mode code		*/
/* devcap : tapecap entry for device		*/
/* devpos : device position info (or NULL ptr)	*/
static struct mtdesc *zmtdesc ( const char *device, int acmode, 
				const char *devcap, struct _mtpos *devpos )
{
	struct mtdesc *mp;
	struct mtdev *dp;
	...
	mp->acmode = acmode;
	safe_strcpy (mp->iodev, SZ_FNAME, device);
...
#ifdef TCPIP
static	signal_handler_t sigpipe = NULL;
...
static void zmtdbg ( struct mtdesc *mp, const char *msg )
{
	FILE *out;
	const char *ip;
