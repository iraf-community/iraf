...
v2start_
	    ...
	    switch (htype) {
	    case TY_IMHDR:
		if (imiiwrc (fp, V2_MAGIC, SZ_IMMAGIC) == ERR)
		    goto v2done_
		hdrlen = LEN_V2IMHDR
	    case TY_PIXHDR:
		if (imiiwrc (fp, V2_PMAGIC, SZ_IMMAGIC) == ERR)
		    goto v2done_
		hdrlen = LEN_V2PIXHDR
	    default:
		goto v2done_
	    ...
	    if (imiiwri (fp, hdrlen, 1) == ERR)
		goto v2done_
	    if (imiiwri (fp, IM_PIXTYPE(im), 1) == ERR)
		goto v2done_
	    ...
	    if (imiiwri (fp, IM_SWAPPED(im), 1) == ERR)
		goto v2done_
	    if (imiiwri (fp, IM_NDIM(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_LEN(im,1), IM_MAXDIM) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_PHYSLEN(im,1), IM_MAXDIM) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_SSMTYPE(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_LUTOFF(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_PIXOFF(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_HGMOFF(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_BLIST(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_SZBLIST(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_NBPIX(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_CTIME(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_MTIME(im), 1) == ERR)
		goto v2done_
	    if (imiiwrl (fp, IM_LIMTIME(im), 1) == ERR)
		goto v2done_
	    if (imiiwrr (fp, IM_MAX(im), 1) == ERR)
		goto v2done_
	    if (imiiwrr (fp, IM_MIN(im), 1) == ERR)
		goto v2done_
		...
		if (imiiwrc (fp, Memc[fname], SZ_V2IMPIXFILE) == ERR)
		    goto v2done_
		status = OK
		goto v2done_
	    } else if (imiiwrc (fp, IM_PIXFILE(im), SZ_V2IMPIXFILE) == ERR)
		goto v2done_

	    call oif_trim (IM_HDRFILE(im), SZ_V2IMHDRFILE)
	    if (imiiwrc (fp, IM_HDRFILE(im), SZ_V2IMHDRFILE) == ERR)
		goto v2done_

	    call oif_trim (IM_TITLE(im), SZ_V2IMTITLE)
	    if (imiiwrc (fp, IM_TITLE(im), SZ_V2IMTITLE) == ERR)
		goto v2done_

	    call oif_trim (IM_HISTORY(im), SZ_V2IMHIST)
	    if (imiiwrc (fp, IM_HISTORY(im), SZ_V2IMHIST) == ERR)
		goto v2done_

	    # Write the variable-length user area.
	    if (imiiwrc (fp, Memc[IM_USERAREA(im)], len_userarea) == ERR)
		goto v2done_
