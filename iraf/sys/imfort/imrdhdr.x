...
bool	streq()
int	imiirdc(), imiirdi(), imiirdl(), imiirdr()
...
begin
	...
	if (bfseek (fp, BOFL) == ERR)
	    return (ERR)
	if (imiirdc (fp, immagic, SZ_IMMAGIC) < 0)
	    return (ERR)
	...
	if (htype == TY_PIXHDR && streq (immagic, V2_PMAGIC)) {
	    ...
	    if (imiirdi (fp, hdrlen, 1) != 1)
		goto readerr_
	    sulen_userarea = hdrlen - LEN_V2IMHDR
	    IM_HDRLEN(im) = LEN_IMHDR + sulen_userarea

	    if (imiirdi (fp, IM_PIXTYPE(im), 1) != 1)
		goto readerr_

	    # Determine whether to byte swap the pixels.
	    if (imiirdi (fp, IM_SWAPPED(im), 1) != 1)
		goto readerr_
	    ...
	    if (imiirdi (fp, IM_NDIM(im), 1) < 0)
		goto readerr_
	    if (imiirdi (fp, IM_LEN(im,1), IM_MAXDIM) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_PHYSLEN(im,1), IM_MAXDIM) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_SSMTYPE(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_LUTOFF(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_PIXOFF(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_HGMOFF(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_BLIST(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_SZBLIST(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_NBPIX(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_CTIME(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_MTIME(im), 1) < 0)
		goto readerr_
	    if (imiirdl (fp, IM_LIMTIME(im), 1) < 0)
		goto readerr_

	    if (imiirdr (fp, IM_MAX(im), 1) < 0)
		goto readerr_
	    if (imiirdr (fp, IM_MIN(im), 1) < 0)
		goto readerr_

	    if (imiirdc (fp, IM_PIXFILE(im), SZ_V2IMPIXFILE) < 0)
		goto readerr_
	    if (imiirdc (fp, IM_HDRFILE(im), SZ_V2IMHDRFILE) < 0)
		goto readerr_
	    if (imiirdc (fp, IM_TITLE(im), SZ_V2IMTITLE) < 0)
		goto readerr_
	    if (imiirdc (fp, IM_HISTORY(im), SZ_V2IMHIST) < 0)
		goto readerr_

	    # Read the variable-length user area.
	    if (uchars > 0) {
		nchars = min (uchars, sulen_userarea * SZ_STRUCT)
		if (imiirdc (fp, Memc[IM_USERAREA(im)], nchars) < 0)
		    goto readerr_
	    }
