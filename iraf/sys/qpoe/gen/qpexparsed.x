...
begin
	vlen = DEF_VLEN
	sz_val = vlen
	call malloc (xv, sz_val, TY_DOUBLE)
	...
	for (ip=1;  expr[ip] != EOS;  ) {
		...
		if (nrg+4 > vlen) {
		    vlen = vlen + INC_VLEN
		    sz_val = vlen
		    call realloc (xv, sz_val, TY_DOUBLE)
...
pop_
	...
	if (xlen <= 0) {
	    xlen = DEF_XLEN
	    sz_val = xlen
	    call malloc (xs, sz_val, TY_DOUBLE)
	    call malloc (xe, sz_val, TY_DOUBLE)
	}
	...
	for (r1=1;  r1 <= nrg;	r1=r2+1) {
		...
		if (op > xlen) {
		    xlen = xlen + INC_XLEN
		    sz_val = xlen
		    call realloc (xs, sz_val, TY_DOUBLE)
		    call realloc (xe, sz_val, TY_DOUBLE)
		}
