...
long	vs[ndim], ve[ndim]	#I vectors defining region to be extracted
...
double	x, y
pointer	sp, evl, ev_p
int	evtype, maskval, xoff, yoff, nev, totev, i
long	xw, yw, maxpix, pix, j
...
begin
	...
	xw = (ve[1] - vs[1] + 1) / xblock + (EPSILOND * 1000)
	yw = (ve[2] - vs[2] + 1) / yblock + (EPSILOND * 1000)
	if (xw <= 0 || yw <= 0)
	    return (0)
	...
	# Read the events.
	while (qpio_getevents (io, Memp[evl], maskval, SZ_EVLIST, nev) > 0) {
	    switch (evtype) {

	    case TY_SHORT:
		# Process a sequence of neighbor events.
		do i = 1, nev {
		    ev_p = (Memp[evl+i-1] - 1) * SZ_SHORT / SZ_SHORT + 1

		    x = Mems[ev_p+xoff]  
		    y = Mems[ev_p+yoff]

		    j = (y - vs[2]) / yblock + (EPSILOND * 1000)
		    if (j >= 0 && j < yw) {
		...
		# Process a sequence of neighbor events.
		do i = 1, nev {
		    ev_p = (Memp[evl+i-1] - 1) * SZ_SHORT / SZ_INT + 1
		...
		# Process a sequence of neighbor events.
		do i = 1, nev {
		    ev_p = (Memp[evl+i-1] - 1) * SZ_SHORT / SZ_LONG + 1
		...
		# Process a sequence of neighbor events.
		do i = 1, nev {
		    ev_p = (Memp[evl+i-1] - 1) * SZ_SHORT / SZ_REAL + 1
		...
		# Process a sequence of neighbor events.
		do i = 1, nev {
		    ev_p = (Memp[evl+i-1] - 1) * SZ_SHORT / SZ_DOUBLE + 1
