...
errchk	qpex_dballoc, malloc, calloc, realloc, qpex_parser
...
null_		call eprintf ("%s: null range list\n")
		    call pargstr (qpexwarn)
		call qpex_r_pbpin_1 (ex, PASS, 0, d_zero, d_zero)
		...
		if (dtype == TY_SHORT) {
			...
			call qpex_r_pbpin_1 (ex, LEQXS, offset, d_x2, d_zero)
		    else if (IS_RIGHTR(x2))
			call qpex_r_pbpin_1 (ex, GEQXS, offset, d_x1, d_zero)
		    else if (fp_equalr (x1, x2))
			call qpex_r_pbpin_1 (ex, EQLXS, offset, d_x1, d_x2)
		    else
			call qpex_r_pbpin_1 (ex, RNGXS, offset, d_x1, d_x2)
		} else {
		    if (IS_LEFTR(x1) && IS_RIGHTR(x2))
			; # pass everything (no tests)
		    else if (IS_LEFTR(x1))
			call qpex_r_pbpin_1 (ex, LEQXR, offset, d_x2, d_zero)
		    else if (IS_RIGHTR(x2))
			call qpex_r_pbpin_1 (ex, GEQXR, offset, d_x1, d_zero)
		    else if (fp_equalr (x1, x2))
			call qpex_r_pbpin_1 (ex, EQLXR, offset, d_x1, d_x2)
		    else
			call qpex_r_pbpin_1 (ex, RNGXR, offset, d_x1, d_x2)
		}
		...
		if (nranges == 2)
			...
			if (fp_equalr (XE(1), XS(2))) {
			    call qpex_r_pbpin_1 (ex, NEQXR, offset, XE(1), d_zero)
			    goto resume_
			}
		...
		if (level == 0) {
			opcode = LDRR
		    call qpex_r_pbpin_1 (ex, opcode, offset, d_zero, d_zero)
		}
		...
		do i = 1, nranges {
		    ...
		    if (IS_LEFTR(x1))
			call qpex_r_pbpin_0 (ex, LEQR, d_x2, d_zero, d_zero)
		    else if (IS_RIGHTR(x2))
			call qpex_r_pbpin_0 (ex, GEQR, d_x1, d_zero, d_zero)
		    else if (fp_equalr (x1, x2))
			call qpex_r_pbpin_0 (ex, EQLR, d_x1, d_x2, d_zero)
		    else
			call qpex_r_pbpin_0 (ex, RNGR, d_x1, d_x2, d_zero)
		}

		# Compile a test and exit instruction.
		call qpex_r_pbpin_1 (ex, XIFF, 0, d_zero, d_zero)
		...
		if (dtype == TY_SHORT)
		    ...
		    call qpex_p_pbpin_1 (ex, LUTXR, offset, lt, p_zero)
...
resume_
		...
		call qpex_r_pbpin_1 (ex, RET, 0, d_zero, d_zero)
