...
begin
	...
	if (status == ERR) {
	    sz_val = SZ_FNAME
	    call strupk (osfn, Memc[fname], sz_val)
...
long procedure osfn_timeleft (osfn, time)
...
long procedure osfn_unlock (osfn, time)
...
begin
	...
	host_osfn[op] = EOS
	sz_val = maxch
	call strpak (host_osfn, host_osfn, sz_val)
end
