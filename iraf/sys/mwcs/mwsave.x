...
size_t	sz_val
size_t	nchars, ml_milen, ml_svhdr
#size_t	olen
pointer	mw, sp, sv, op
#pointer	oo
...
begin
	...
	MI_REFIM(mw) = NULL
	sz_val = MAX_DIM
	call aclri (MI_AXNO(mw,1), sz_val)
	call aclri (MI_AXVAL(mw,1), sz_val)
	call aclri (MI_PHYSAX(mw,1), sz_val)
...
#	ml_milen = olen * SZ_SHORT

	ml_svhdr = LEN_SVHDR * 4	# 8-bytes: for miipak64()
	ml_milen = MI_LEN(mw) * 4	# 8-bytes: for miipak64()
	nchars = ml_svhdr + ml_milen +
...
#	SV_CWCSLEN(sv) = olen
	...
	SV_MWSVOFF(sv) = ml_svhdr
	SV_DBUFOFF(sv) = (SV_MWSVOFF(sv) + ml_milen + SZ_DOUBLE-1) /
			    SZ_DOUBLE * SZ_DOUBLE
	...
	op = coerce (bp, TY_CHAR, TY_POINTER)
	sz_val = LEN_SVHDR
	call miipak64 (Memp[sv], Memp[op], sz_val, TY_STRUCT)
...
#	call miipak16 (Mems[oo], Mems[op], olen, TY_SHORT)
	op = coerce (bp + SV_MWSVOFF(sv), TY_CHAR, TY_POINTER)
	sz_val = MI_LEN(mw)
	call miipak64 (Memp[mw], Memp[op], sz_val, TY_STRUCT)
