...
begin
	...
	if (miireadp (fd, Memp[sv], sz_val) != LEN_SVDES)
	    goto err_
	...
	if (miireadc (fd, Memc[text], sz_val) != SV_TITLELEN(sv))
	    goto err_
	else
	...
	sz_val = SV_MASKLEN(sv) * SZ_SHORT
	iferr (nchars = read (fd, Mems[bp], sz_val))
	    goto err_
	call close (fd)
