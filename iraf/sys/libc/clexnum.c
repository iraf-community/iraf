 ...
 */
/* str    : input string    */
/* toklen : nchars in token */
int c_lexnum ( const char *str, int *toklen )
{
	const char *ip;
	XCHAR *op, *maxop;
	PKCHAR numbuf[SZ_FNAME];
	XINT ip_start = 1;
	XINT x_status, x_toklen = *toklen;
	int ndigits;
	 ...
	 */
	ndigits=0;
	maxop = numbuf + SZ_FNAME -1;
	for ( ip=str, op=numbuf ; op < maxop && (*ip) ; op++, ip++ ) {
	    *op = *ip;
	    if (isdigit (*ip)) ndigits++;
	}
	*op = XEOS;
	...
	if (ndigits == (ip - str)) {
	    *toklen = ndigits;
	    ...
	    x_status = LEXNUM (numbuf, &ip_start, &x_toklen);
	    *toklen = x_toklen;
	    return x_status;
	}
}
