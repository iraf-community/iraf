...
long procedure miireadd (fd, spp, maxelem)
...
begin
	...
	if (pksize > maxelem * SZ_DOUBLE) {
	    ...
	    if (nchars != EOF) {
		sz_val = nchars
		nelem = min (maxelem, miinelem (sz_val, MII_DOUBLE))
		sz_val = nelem
		call miiupkd (Memc[bp], spp, sz_val, TY_DOUBLE)
	    }
		...
		nelem = min (maxelem, miinelem (sz_val, MII_DOUBLE))
		sz_val = nelem
		call miiupkd (spp, spp, sz_val, TY_DOUBLE)
	    }
	}
