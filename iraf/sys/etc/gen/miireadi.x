...
long procedure miireadi (fd, spp, maxelem)
...
begin
	...
	if (pksize > maxelem * SZ_INT) {
	    ...
	    if (nchars != EOF) {
		sz_val = nchars
		nelem = min (maxelem, miinelem (sz_val, MII_INT))
		sz_val = nelem
		call miiupki (Memc[bp], spp, sz_val, TY_INT)
	    }
		...
		nelem = min (maxelem, miinelem (sz_val, MII_INT))
		sz_val = nelem
		call miiupki (spp, spp, sz_val, TY_INT)
	    }
	}
