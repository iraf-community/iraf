...
# FITS card dimensions

define	SZ_KEYWORD	8	# max keyword name length
define	LEN_OBJECT	63	# max string length
...
begin
	...
	if (ndim != 0) {
	   call sprintf (keyname, SZ_KEYWORD, "DATAMIN")
	   call wft_encodec (keyname, zero, LEN_CARD, card, "")
	   card[LEN_CARD+1] = '\n'
	   card[LEN_CARD+2] = EOS
	...
	do k = 1, ndim {
	   call sprintf (keyname, SZ_KEYWORD, "CRVAL%d")
	        call pargi(k)	       
	   call wft_encodec (keyname, one, LEN_CARD, card, "")
	   card[LEN_CARD+1] = '\n'
	   card[LEN_CARD+2] = EOS
	   call putline (fd, card)
	   ...
	   call sprintf (keyname, SZ_KEYWORD, "CTYPE%d")
	        call pargi(k)	       
	   call wft_encodec (keyname, "PIXEL", LEN_CARD, card, "")
	   card[LEN_CARD+1] = '\n'
	   card[LEN_CARD+2] = EOS
	   call putline (fd, card)
