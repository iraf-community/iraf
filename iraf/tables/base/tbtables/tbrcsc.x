...
long	irow		# i: row number in input table
long	orow		# i: row number in output table
int	ncols		# i: number of columns to be copied
#--
pointer sp
int	k		# loop index for column number
long	i		# loop index for array element
long	nget, nput	# number of elements in input & output arrays
...
size_t	sz_val
long	c_1
long	tbagtd(), tbagtr(), tbagti(), tbagts(), tbagtt()
...
begin
	...
	do k = 1, ncols {
		...
		case TBL_TY_REAL:
		    ...
		    sz_val = max (nget, nput)
		    call malloc (gbuf, sz_val, TY_REAL)
		    do i = nget+1, nput
			Memr[gbuf+i-1] = INDEFR
		    if (tbagtr (itp, icp[k], irow, Memr[gbuf], c_1, nget) < nget)
			call error (1, ERR_READ_ARRAY)
		    call tbaptr (otp, ocp[k], orow, Memr[gbuf], c_1, nput)
		    call mfree (gbuf, TY_REAL)

		case TBL_TY_DOUBLE:

		    sz_val = max (nget, nput)
		    call malloc (gbuf, sz_val, TY_DOUBLE)
		    do i = nget+1, nput
			Memd[gbuf+i-1] = TBL_INDEFD
		    if (tbagtd (itp, icp[k], irow, Memd[gbuf], c_1, nget) < nget)
			call error (1, ERR_READ_ARRAY)
		    call tbaptd (otp, ocp[k], orow, Memd[gbuf], c_1, nput)
		    call mfree (gbuf, TY_DOUBLE)

		case TBL_TY_INT:

		    sz_val = max (nget, nput)
		    call malloc (gbuf, sz_val, TY_INT)
		    do i = nget+1, nput
			Memi[gbuf+i-1] = INDEFI
		    if (tbagti (itp, icp[k], irow, Memi[gbuf], c_1, nget) < nget)
			call error (1, ERR_READ_ARRAY)
		    call tbapti (otp, ocp[k], orow, Memi[gbuf], c_1, nput)
		    call mfree (gbuf, TY_INT)

		case TBL_TY_SHORT,TBL_TY_BOOL:

		    sz_val = max (nget, nput)
		    call malloc (gbuf, sz_val, TY_SHORT)
		    do i = nget+1, nput
			Mems[gbuf+i-1] = INDEFS
		    if (tbagts (itp, icp[k], irow, Mems[gbuf], c_1, nget) < nget)
			call error (1, ERR_READ_ARRAY)
		    call tbapts (otp, ocp[k], orow, Mems[gbuf], c_1, nput)
		...
		default:
			...
			do i = 1, nget {
				...
				Memc[cbuf], SZ_LINE, i, c_1) < 1)
				    call error (1, ERR_READ_ARRAY)
			    call tbaptt (otp, ocp[k], orow,
				Memc[cbuf], SZ_LINE, i, c_1)
			}
			do i = nget+1, nput
			    call tbaptt (otp, ocp[k], orow, "", SZ_LINE, i, c_1)
		    } else {
			call error (ER_TBCOLBADTYP, BAD_DATATYPE)
		    }
		}
