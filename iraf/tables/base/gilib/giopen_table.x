...
pointer	colptr[ARB]			# o: pointer to each column
...
begin
	...
	tp = tbtopn (tname, NEW_FILE, NULLPTR)
	...
	do i = 1, pcount {
	   ...
	   case TY_SHORT:	
		...
		call tbcdef (tp, colptr[i], P_PTYPE(pp), P_PDTYPE(pp), "", 
			     TY_SHORT, c_1, 1)
	   case TY_INT:
		call tbcdef (tp, colptr[i], P_PTYPE(pp), "", "", TY_INT, c_1, 1)
	   case TY_LONG:
		call tbcdef (tp, colptr[i], P_PTYPE(pp), "", "", TY_LONG, c_1, 1)
	   case TY_REAL:
		call tbcdef (tp, colptr[i], P_PTYPE(pp), "", "", TY_REAL, c_1, 1)
	   case TY_DOUBLE:
