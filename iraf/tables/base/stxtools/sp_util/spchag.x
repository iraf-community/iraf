...
procedure sp_change_string( sinput, old, new, output, max )

char sinput[ARB]  # I:  The input string.
...
begin
  ...
  while( ip < ( ilen + 1 ) && op < ( max + 1 ) ) {
    ...
    after = gstrmatch( sinput[ip], old, first, last )

    # If the string is not found, then copy the rest of the input to the
    # output.
    if( after == 0 ) {
      call strcpy( sinput[ip], output[op], max - op + 1 )
      ip = ilen + 1
    }
    ...
    # and replace the old string.
      ...
      call sp_strncpy( sinput[ip], first, output[op] )
      ip = ip + last
      op = op + first
