...
int	naperts
size_t	cboxsize, rboxsize, npts, nbins, sz_val
long	l_val, c_2
...
long	dp_rprofile(), lnint(), lint(), lmod()
int	strlen(), dp_gaperts(), btoi()
bool	clgetb()
...
begin
	...
	rboxsize = 2 * lnint (pradius + 1.0) + 1
	call smark (sp)
	...
	# radius.
	nbins = lint (pradius / 0.5 + 0.5)
	call salloc (nr, nbins, TY_LONG)
	call salloc (rcentroid, nbins, TY_REAL)
	call salloc (pmean, nbins, TY_REAL)
	call salloc (integral, nbins, TY_REAL)
	call dp_crprofile (Memr[radius], Memr[intensity], npts, Meml[nr],
	...
	sz_val = 1
	call dp_ltov (im, xcenter, ycenter, xcenter, ycenter, sz_val)
...
size_t	boxsize			# width of the centering box
real	xcntr, ycntr		# centered coordinates

long	half_box, x1, x2, y1, y2, ncols, nrows
size_t	nx, ny
int	try
...
begin
	...
	repeat {
	    ...
	    try = try + 1
	    if (try == 1) {
		if ((aabs (xcntr - xinit) > 1.0) || (aabs (ycntr - yinit) >
		    1.0)) {
		    xinit = xcntr
		    yinit = ycntr
		}
...
real	intensity[ARB]		# the output intensity vector
size_t	npts			# number of points in the profile
long	nr[ARB]			# the number of points in each interval
...
size_t	nbins			# the number of radius bins
...
long procedure dp_rivectors (a, nx, ny, x1, y1, xcntr, ycntr, pradius,
	radius, intensity)
