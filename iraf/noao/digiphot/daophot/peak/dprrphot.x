...
int procedure dp_rrphot (tp, key, fields, indices, p_indices, id, x, y, sky, mag,
	instar, nrow)

pointer	tp		# the input file descriptor
pointer	key		# pointer to text apphot structure
char	fields[ARB]	# character fields
int	indices[ARB]	# columns pointers
pointer	p_indices[ARB]	# columns pointers
int	id		# star id
real	x		# x center value
real	y		# y center value
real	sky		# sky value
real	mag		# magnitude
long	instar		# current record
long	nrow		# maximum number of rows for ST table
...
begin
	...
	if (nrow == 0) {
	    i_val = tp
	    nrec = dp_apsel (key, i_val, fields, indices, id, x, y, sky, mag)
	    if (nrec != EOF)
		instar = instar + 1
	    ...
	    instar = instar + 1
	    call tbrgti (tp, p_indices[1], id, nullflag, 1, instar)
	    call tbrgtr (tp, p_indices[2], x, nullflag, 1, instar)
	    call tbrgtr (tp, p_indices[3], y, nullflag, 1, instar)
	    call tbrgtr (tp, p_indices[4], mag, nullflag, 1, instar)
	    call tbrgtr (tp, p_indices[5], sky, nullflag, 1, instar)
	    nrec = instar
