...
int	limlist, lclist, lolist, out, cl, pfd, root, stat, memstat, wcs
long	sid, lid, l_val
...
int	strlen(), apsky(), open(), clgwrd(), ap_memstat()
int	sizeof()
bool	clgetb(), streq()
errchk	gopen
...
begin
	...
	if (verify == YES && interactive == NO) {
	    l_val = 1
	    call ap_sconfirm (ap, NULL, l_val)
	    if (update == YES)
		call ap_pspars (ap)
	}
	...
	while (imtgetim (imlist, Memc[image], SZ_FNAME) != EOF) {
	    ...
	    if (interactive == NO) {
	        if (Memc[cname] != EOS)
		    stat =  apsky (ap, im, cl, NULL, NULLPTR, mgd, NULLPTR, out,
		        sid, NO, cache)
	        else if (cl != NULL) {
		    lid = 1
