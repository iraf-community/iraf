...
long procedure ap_grow_hist (skypix, coords, wgt, nskypix, index, snx, sny,
			     hgm, nbins, z1, z2, rgrow)

real	skypix[ARB]		# sky pixels
long	coords[ARB]		# sky pixel coordinates
...
long	j, k, ixc, iyc, ymin, ymax, xmin, xmax, nreject, cstart, c, bin, l_val
real	dh, r2, rgrow2, d
long	lmod(), lint()
	...
	do j = ymin, ymax {
		...
		if (r2 <= rgrow2 && c == coords[cstart] && wgt[cstart] > 0.0) {
		    ...
		    if (skypix[cstart] >= z1 && skypix[cstart] <= z2) {
		        bin = lint ((skypix[cstart] - z1) * dh) + 1
		        hgm[bin] = hgm[bin] - 1.0
		    }
		}
	    }
...
long procedure ap_grow_hist2 (skypix, coords, wgt, nskypix, sky_zero, index,
    snx, sny, hgm, nbins, z1, z2, rgrow, sumpx, sumsqpx, sumcbpx)

real	skypix[ARB]		# sky pixels
long	coords[ARB]		# coordinates
real	wgt[ARB]		# array of weights
size_t	nskypix			# number of sky pixels
