...
define	PINP_COL	Memi[P2I($1+0)]		# input column
define	PINP_ERRCOL	Memi[P2I($1+1)]		# error column
define	PINP_WTSCOL	Memi[P2I($1+2)]		# weight column
define	PINP_SPARE	Memi[P2I($1+3)]		# spare flag (YES/NO)
...
define	PSEQ_EQ		Memi[P2I($1 + 0)]	# equation string offset
define	PSEQ_ERROR	Memi[P2I($1 + 1)]	# error equation string offset
define	PSEQ_ERRMIN	Memi[P2I($1 + 2)]	# min error equation string offset
define	PSEQ_ERRMAX	Memi[P2I($1 + 3)]	# max error equation string offset
...
define	PTEQ_NRCAT	Memi[P2I($1 + 0)]
define	PTEQ_NROBS	Memi[P2I($1 + 1)]
define	PTEQ_NRVAR	Memi[P2I($1 + 2)]
# - number of catalog and observation variables, and total number of variables
#   for fit equation.
define	PTEQ_NFCAT	Memi[P2I($1 + 3)]
define	PTEQ_NFOBS	Memi[P2I($1 + 4)]
define	PTEQ_NFVAR	Memi[P2I($1 + 5)]
# - total number of variables in the reference and fit equations.
define	PTEQ_NVAR	Memi[P2I($1 + 6)]
...
define	PTEQ_NPAR	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 7)]
define	PTEQ_NFPAR	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 8)]
...
define	PTEQ_FIT	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 9)] 
define	PTEQ_REF	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 10)]
# - error, minimum error, and maximum error equation string offsets
define	PTEQ_ERROR	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 11)]
define	PTEQ_ERRMIN	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 12)]
define	PTEQ_ERRMAX	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 13)]
# - weight, minimum weight, and maximum weight equation string offsets
define	PTEQ_WEIGHT	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 14)]
define	PTEQ_WTSMIN	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 15)]
define	PTEQ_WTSMAX	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 16)]
# - plot equation string offsets
define	PTEQ_XPLOT	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 17)]
define	PTEQ_YPLOT	Memi[P2I($1 + 2 * PTEQ_NVAR($1) + 3 * PTEQ_NPAR($1) + 18)]
...
# Vector access
define	PTEQ_AREFVAR	Memi[P2I(PTEQ_SREFVAR($1))]	    # ref. variable sym. off.
define	PTEQ_AREFCNT	Memi[P2I(PTEQ_SREFCNT($1))]	    # ref. variable counters
define	PTEQ_AFITVAR	Memi[P2I(PTEQ_SFITVAR($1))]	    # fit variable sym. off.
define	PTEQ_AFITCNT	Memi[P2I(PTEQ_SFITCNT($1))]	    # fit variable counters
define	PTEQ_APAR	Memi[P2I(PTEQ_SPAR($1))]	    # parameter sym. offsets
...
define	PTEQ_REFCNT	Memi[P2I(PTEQ_SREFCNT($1)) + $2 - 1]	# ref. var. counter
define	PTEQ_FITVAR	Memi[P2I(PTEQ_SFITVAR($1)) + $2 - 1] # fit var. symbol off.
define	PTEQ_FITCNT	Memi[P2I(PTEQ_SFITCNT($1)) + $2 - 1] # fit var. counter
...
define	PTEQ_PAR	Memi[P2I(PTEQ_SPAR($1))    + $2 - 1] # symbol offset
define	PTEQ_PARVAL	Memr[P2R(PTEQ_SPARVAL($1)) + $2 - 1] # value
define	PTEQ_PLIST	Memi[P2I(PTEQ_SPLIST($1))  + $2 - 1] # list element

# Individual access for derivative equation string offsets and codes. The
# second argument is the parameter number relative to the equation.
define	PTEQ_DER	Memi[P2I(PTEQ_SDER($1))    + $2 - 1] # string offset
define	PTEQ_RPNDER	Memi[P2I(PTEQ_SRPNDER($1)) + $2 - 1] # code
