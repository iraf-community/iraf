...
int   vot_regResolver (char *id, char *svctype, char *bpass, char *subject,
		    	char *fields, int index, int exact, int dalOnly,
		    	char **result);
		 ...
		 char *fields, int index, int exact, int dalOnly, char **res)
{
    ...
    char      qstring[SZ_QSTRING], *svcstr, *bpstr, *substr, *buf;
		   ...
		   int sortRes, int terse)
{
    ...
    */
    ...
    if (!keywOnly || nids == 0) {
        ...
        if (substr) {			/* add subject constraint	*/
	    if (qstring[0]) {
                strcat (qstring,  (orValues ? " OR " : " AND "));
...
*/
    ...
    */
    if (nresults == 0 && (!svcstr && !bpstr) && !haveADQL) {
        if (ids[0]) {
	    bzero (qstring, SZ_QUERY);
            for (i=0; i < nids; i++) {
		if (qstring[0])
    ...
    */
    ...
    if (votable) {
        for (i=0; i < nresults; i++) {
	    vot_printRegVOTableRec (stdout, res, i);
	}
...
vot_printAttrs (char *fname, Query query, char *ident)
{
    ...
    if ((ip = strstr (qstring, "format=image")) ||
        (ip = strstr (qstring, "FORMAT=image"))) {
	    op = ip;
	    while (*ip && *ip != '&') 
	        ip++;
	    if (*ip) {
    ...
    */
	...
	char ucd[SZ_FNAME], name[SZ_FNAME], desc[SZ_LINE], id[SZ_FNAME];
...
*/
	...
	if (!fname || strcmp (fname, "-") == 0) {
	    ...
	    strcpy (ucd,  vot_getAttr (field, "ucd"));
	    strcpy (name, vot_getAttr (field, "name"));
	    strcpy (id,   vot_getAttr (field, "id"));
	    strcpy (desc, vot_getAttr (field, "description"));
