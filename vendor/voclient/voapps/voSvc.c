...
static int vot_isSupportedSvc (char *type);
...
vot_loadResourceVOTable (char *fname)
{
	...
	for (i=0; i < nr; i++) {
	    ...
	    bzero (name,  SZ_FNAME); 
	    strcpy (name,  vot_getTableCell(vot, i, c_name));

	    bzero (url,   SZ_LINE); 
	    strcpy (url,   vot_getTableCell(vot, i, c_url));

	    bzero (id,    SZ_FNAME); 
	    strcpy (id,    vot_getTableCell(vot, i, c_id));

	    bzero (type,  SZ_FNAME); 
	    strcpy (type,  vot_getTableCell(vot, i, c_type));
	    ...
	    bzero (title, SZ_LINE); 
	    strcpy (title, vot_getTableCell(vot, i, c_title));
...
{
    int   vot = 0, nr, nc, res, tab, field, data, tdata;
    char  name[SZ_FNAME];
    ...
    */
	...
	nc = vot_getNCols (tdata);
	    ...
	    bzero (name, SZ_FNAME);
	    strcpy (name, vot_getAttr (vot, "name"));
		    ...
		    if (strcasecmp ("any", id) == 0) {
		        if (typestr) {
		            use_any = 1;
		                ...
		                fields, -1, all_data, dalOnly, &result);
			    nservices += nres;
		        } else if (inventory) {
			    nservices = -1;
		        } else {
