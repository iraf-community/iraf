...
2. A right arrow symbol (\verb+>+) is used in the function descriptions to
...
The in-range operator, (X=A:B), tests whether X is in the range
delimited by A to B.  Specifically, the in-range operator evaluates
to true if \verb+(X >= A && X <= B)+.
...
\end{verbatim}
    ...
    more  bit  fields  using  the  '\verb+&+'(AND),  '\verb+|+'(OR),  and the '\verb+!+'(NOT)
