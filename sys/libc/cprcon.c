...
int	*out;			/* FD for writing to child		*/
{
	...
	iferr (pid = PROPEN (c_sppstr(process), in, out))
	    return (NULL);
	else {
 ...
 */
c_prclose (pid)
unsigned pid;			/* process id returned by C_PROPEN	*/
{
	return (PRCLOSE (&pid));
}
...
{
	iferr (PRREDIR (&pid, &stream, &new_fd))
	    return (ERR);
	else
	    return (OK);
}
...
char	*newdir;
{
	return (PRCHDIR (&pid, c_sppstr (newdir)));
}
