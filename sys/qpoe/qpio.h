...
define	KEYWORDS	"|block|debug|filter|key|noindex|param|mask|rect|"

define	KW_BLOCK	1		# blocking factor for image matrix
define	KW_DEBUG	2		# debug level (integer, 0=nodebug)
define	KW_FILTER	3		# event attribute filter
define	KW_KEY		4		# event key (Y,X) fields (e.g.(s10,s8))
define	KW_NOINDEX	5		# don't use index even if present
define	KW_PARAM	6		# name of event list header parameter
define	KW_MASK		7		# region mask
define	KW_RECT		8		# rectangle (bounding box) for i/o
...
# index
define	IO_INDEXLEN	Memi[$1+40]	# length of index (same as nlines)
define	IO_IXXOFF	Memi[$1+41]	# offset of X field used in index
define	IO_IXYOFF	Memi[$1+42]	# offset of Y field used in index
