...
begin
	...
	if (IM_PL(im) != NULL) {
	    ...
	    # "Fast i/o" in the conventional sense no IMIO buffering) is not
	    # permitted for mask images, since IMIO must buffer the pixels,
	    # which are generated at run time.

	    IM_PFD(im) = open ("dev$null", READ_WRITE, BINARY_FILE)
