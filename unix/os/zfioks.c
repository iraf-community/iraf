...
d_err:		dbgmsg ("S:in.irafksd parent exit, status=%d\n", status);
	    ...
	    dbgmsg ("S:in.irafksd started, pid=%d ppid=%d\n",
		getpid(), getppid());
		...
		if ((sig = setjmp(jmpbuf))) {
	    	    if (sig == SIGCHLD) {
	    		dbgstr ("S:in.irafksd sigchld return\n");
...
s_err:		    dbgmsg ("S:in.irafksd fork complete, status=%d\n",
		    ...
		    dbgmsg ("S:irafks server started, pid=%d ppid=%d\n",
			getpid(), getppid());
...
r_err:		dbgstr ("rexec-callback connect failed\n");
		close(s);  close(ss);
		*chan = ERR;
	    } else {
...
retry:
		...
		if (port && status == UNAUTH) {
		    close(t);
		    port = 0;
		    dbgstr ("C:authorization failed, retry with port=0\n");
		    status = 0;
		    goto again;
		} else {
		    status |= 020000;
...
c_err:		dbgmsg ("C:zfioks client status=%o\n", status);
	...
	dbgmsg ("zopnks returns status=%d\n", *chan);
...
)
{
	...
	*status = close (*chan);
	...
	dbgmsg ("server [%d] terminated, status = %d\n", *chan, *status);
...
)
{
	    ...
	    dbgmsg ("ks_reaper -- pid=%d, status=%d\n", pid, status);
...
ks_geti (int fd)
{
	...
	do {
		...
		dbgstr ("ks_geti: timeout on read\n");
		jmpset = 0;
		return (ERR);
	    }
...
ks_gets (int fd, char *outstr)
{
	...
	do {
		...
		dbgmsg ("ks_gets: read status=%d, errno=%d (%s)\n", 
		    stat, errno, strerror(errno));
		return (ERR);
	    }
	} while (*op++);
...
dbgmsg (char *format, ...)
{
        if (debug_ks && debug_fp != (FILE *) NULL) {
            int pid;
            va_list vargs;
            ...
            vfprintf (debug_fp, format, vargs);
            va_end(vargs);
