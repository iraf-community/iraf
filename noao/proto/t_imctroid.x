# IMCENTROID.  This should be revised to handle more than two dimensions
# and to read the coordinates and shifts directly from the parameters.
...
# images, optionally find the average shifts relative to a reference.
...
begin
	...
	iferr {
	    ...
	    if (VERBOSE(cp) == YES) {
		call printf (
		    "#%16timage   X-center  err    Y-center  err    num\n")
		call flush (STDOUT)
	    }
		...
		do j = 1, ncoords {
		    ...
		    if (firsttime) {
			if (VERBOSE(cp) == YES) {
			    call printf ("#%12treference   X-center  err")
		    ...
		    if (VERBOSE(cp) == YES) {
			call printf (
			    "%20s   %7.2f (%0.2f)   %7.2f (%.2f) %4d\n")
			    call pargstr (IM_HDRFILE(ref))
...
# IA_COLSUM -- Sum all columns in a raster.
...
# IA_CNTR1D -- Compute the "center of gravity" (the first moment).
...
# IA_OPENP2R -- open a list file (in the future as a template) from
# which two real values per line are expected.

# The mechanism should be generalized for different datatypes and
...
begin
	...
	# whitespace?
#	if (Memc[fname] == EOS || strmatch (Memc[fname], "^#$") != 0) {
...
# IA_STATS -- actually calculate the shifts.
...
begin
	...
	for (i=1; imtgetim (imlist, Memc[img], SZ_FNAME) != EOF; i=i+1) {
	    ...
	    if (firsttime) {
		call printf ("#!%16timage    X-shift  err     ")
...
# IA_TRIM -- calculate the trim section.
...
begin
	...
	if (vxlo != ixlo || vxhi != ixhi || vylo != iylo || vyhi != iyhi) {
	    call eprintf ("# The overlap region is vignetted!\n")
	    call eprintf ("%d %d %d %d %24t# vig. section = [%d:%d,%d:%d]\n")
		call pargi (vxlo)
		call pargi (vxhi)
		call pargi (vylo)
		call pargi (vyhi)
	...
	call printf ("%d %d %d %d %24t# trim section = [%d:%d,%d:%d]\n")
	    call pargi (ixlo)
	    call pargi (ixhi)
	    call pargi (iylo)
	    call pargi (iyhi)
