...
# IR_SHIFTS -- Procedure to compute the shifts for each subraster.

int procedure ir_shifts (cl, xrshift, yrshift, xcshift, ycshift, nrshift,
...
begin
	...
	while (fscan (cl) != EOF) {
	    ...
	    # This is an illegal shift, the subrasters are the same or not
	    # adjacent.
	    #if (nx1 == nx2 && ny1 == ny2)
		#next
	    #if (nx1 != nx2 && ny1 != ny2)
...
# IR_CSHIFTS -- Procedure to compute the shifts for each subraster.

procedure ir_cshifts (xrshift, yrshift, xcshift, ycshift, nxrsub, nyrsub,
...
begin
	do j = 1, nysub {
		...
		yrshift[i,j] = 0.0 #jsign * abs (yshift)
		xcshift[i,j] = 0.0 #isign * abs (xshift)
