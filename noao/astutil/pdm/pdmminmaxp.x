...
begin
	...
	do i = 1, npt-1 {
	    ...
	    if (Memr[sep+i-1] < minp)
		minp = Memr[sep+i-1]
	}

	# Set minp equal to twice this minimum (Nyquist criterion).  Set fmax.
	PDM_PMIN(pdmp) = 2.0 * minp
	if (PDM_PMIN(pdmp) != 0.0)
	    PDM_FMAX(pdmp) = 1.0/PDM_PMIN(pdmp)

	# Set maxp equal to 4 times maxp.  Set fmin.
	PDM_PMAX(pdmp) = 4.0 * maxp
	if (PDM_PMAX(pdmp) != 0.0)
	    PDM_FMIN(pdmp) = 1.0/PDM_PMAX(pdmp)
