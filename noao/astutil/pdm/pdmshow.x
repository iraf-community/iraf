...
begin
	...
	call fprintf (fd, "minimum period searched = %g, maximum = %g\n")
	    call pargr (PDM_PMIN(pdmp))
	    call pargr (PDM_PMAX(pdmp))
	call fprintf (fd, "period = %g, amplitude = %g, epoch = %g\n")
	...
	if (verbose) {
	    ...
	    do i = 1, PDM_NPT(pdmp) {
		call fprintf ( fd, "index = %d, x = %g, y = %g\n")
		    call pargi (i)
		    call pargr (PDM_X(pdmp,i))
		    call pargr (PDM_DY(pdmp,i))
	    }
	    ...
	    if (PDM_XPHP(pdmp) != NULL) {
		    ...
		    call fprintf ( fd, "index = %d, x = %g, y = %g\n")
		        call pargi (i)
		        call pargr (PDM_XPH(pdmp,i))
		        call pargr (PDM_YPH(pdmp,i))
	        }
	    }
	}
