...
int	i, j, ps, pe
int	ntotal, nreplace, nreject, nindef, nsigma
real	sum1, sum2, scale, sigma
real	var, wt, residual, resid_min, resid_max
pointer	sp, a, b, c, d, e, f
...
begin
	...
	call salloc (e, naps, TY_REAL)
	call salloc (f, naps, TY_INT)
	...
	do j = 1, naps {
		...
		# exit the loop.  Ignore INDEF pixels.
			...
			# the residual limits.
		        ...
		        if (nreplace == EX_NCLEAN(ex))
			    break
		    }
	        } until ((nreplace == EX_NCLEAN(ex)) || (nreject == 0) ||
		    (sum2 == 0.))
	    }
