...
# AP_OFNL -- Procedure to decode the template.

pointer procedure ap_ofnl (ap, template)

pointer	ap			# image descriptor
char	template[ARB]		# field name template

int	tp, nstr, ch, junk
pointer	sp, ip, op, rop, fn, sbuf, pattern, patcode, ranges, nextch
...
begin
	...
	while (template[tp] != EOS && template[tp] != '\n') {
		...
		if (patmatch (Memc[ip], Memc[patcode]) > 0) {
		    call ap_fnputkey (Memc[ip], Memc[ranges], FN_STRP(fn,1),
		        nstr, nextch, sbuf)
		}
...
# AP_GNFN -- Get the next field name matching the given template from the
# apphot data base.

int procedure ap_gnfn (fn, outstr, ranges, maxch)
...
# AP_CFNL -- Procedure to close the list.

procedure ap_cfnl (fn)
...
# AP_FNPUTKEY -- Put a keyword into the keyword list.

procedure ap_fnputkey (key, ranges, strp, nstr, nextch, sbuf)
