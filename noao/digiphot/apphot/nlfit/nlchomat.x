...
# NL_CHOFAC -- Routine to calculate the Cholesky factorization of a
...
procedure nl_chofac (matrix, nbands, nrows, matfac, ier)
...
begin
	...
	do n = 1, nrows {
	    ...
	    if (((matfac[1,n] + matrix[1,n]) - matrix[1,n]) <= EPSILON) {
		do j = 1, nbands
		    matfac[j,n] = 0.
		ier = SINGULAR
		next
...
# NL_CHOSLV -- Solve the matrix whose Cholesky factorization was calculated in
# NL_CHOFAC for the coefficients. This routine was adapted from bchslv.f
# described in "A Practical Guide to Splines", by Carl de Boor (1978).

procedure nl_choslv (matfac, nbands, nrows, vector, coeff)
...
procedure nl_damp (inmatrix, outmatrix, constant, nbands, nrows)
