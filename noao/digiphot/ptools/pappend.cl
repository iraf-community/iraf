# PAPPEND -- Concatenate a list of apphot/daophot databases into a single
# output database. The input files must either be all text files or all data
# files. APPEND checks that the input files were all created by the same
# task.

procedure pappend (infiles, outfile)
...
begin
	...
	if (table)
	    tbappend (in, out, task=task)
	else if (text)
	    txappend (in, out, task=task)
