...
define	MIN_BANDS	-6.0
define	MAX_BANDS	19.0
define	MID_BANDS	15.0
...
begin
	...
	magval = max (minmag, MIN_BANDS)
	dmag = (min (maxmag, MAX_BANDS) - magval) / (nsample - 1)
	do i = 1, nsample {
	    Memr[m+i-1] = magval
	    if (magval > MID_BANDS)
		mtemp = MID_BANDS - mstar
	    else
	        mtemp = magval - mstar
...
procedure st_salpeter (mag, nstars, minmag, maxmag, nsample, order, seed)
...
begin
	...
	# Compute the probability function.
	magval = max (minmag, MIN_SALPETER)
	dmag = (min (maxmag, MAX_SALPETER) - magval) / (nsample - 1)
	do i = 1, nsample {
	    Memr[m+i-1] = magval
...
procedure st_schecter (mag, nstars, alpha, mstar, minmag, maxmag, nsample,
	order, seed)
...
# on their magnitudes. The relation used is that luminosity is roughly
# proportional to diameter squared with a factor of 2 dispersion in
# luminosity. The dispersion relation is uniform.

procedure st_diameters (mag, axis, nstars, minmag, maxmag, minaxis, seed)
...
real	minaxis			# diameter of the faintest galaxy
long	seed			# seed for random number generator
...
begin
	call smark (sp)
	call salloc (errors, nstars, TY_REAL)

	do i = 1, nstars {
	    axis[i] = minaxis * 10.0 ** (0.2 * (maxmag - mag[i]))
	    Memr[errors+i-1] = urand (seed)
	}
	call amapr (Memr[errors], Memr[errors], nstars, 0.0, 1.0,
	    1.0 / sqrt (2.0), sqrt (2.0))
	call amulr (axis, Memr[errors], axis, nstars)
