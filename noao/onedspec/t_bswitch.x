...
int	sfd, nrecsx
int	infile, nfiles, nrecs, def_beam, start_rec, nimage, sub_rate
int	records[3,MAX_RANGES], beam_stat[MAX_NR_BEAMS], ncols[MAX_NR_BEAMS]
real	latitude
...
int	clpopni(), clplen(), clgeti(), get_next_image(), decode_ranges()
int	open(), mod()
real	clgetr()
pointer	immap()
bool	clgetb(), streq()
...
begin
	...
	call smark (sp)
	call salloc (ids[def_beam], LEN_IDS, TY_STRUCT)
	call salloc (POINT(ids[def_beam]), MAX_NCOEFF, TY_REAL)
	...
	while (get_next_image (infile, records, nrecs, image[1,def_beam],
		...
		weight, nrecsx, latitude, ncols, title, imnames)
...
int	dum_beam
bool	idsmode, extinct, weight, exflag
real	latitude
...
pointer	wave_tbl, extn_tbl, ipacc, ipc
...
begin
	...
	# Access pixels - default to the OBJECT default accumulator
	if (IM_NDIM(imin) == 1)
	    accumo[def_beam] = imgl1r (imin,1)
	else
	    accumo[def_beam] = imgl2r (imin,1)

	# Load header area
	call load_ids_hdr (ids[def_beam], imin, 1)
	...
	if (IS_INDEFI (beam_stat[beam_nr])) {
	    ...
	    call salloc (ids   [beam_nr], LEN_IDS,      TY_STRUCT)
	    call salloc (POINT(ids[beam_nr]), MAX_NCOEFF, TY_REAL)
	...
	if (OFLAG(ids[def_beam]) == 1 || !idsmode) {
	    ...
	    call load_ids_hdr (ids[beam_nr], imin, 1)

	    # Fix airmass if necessary
	    if (extinct && IS_INDEF (AIRMASS(ids[beam_nr])))
	...
	if (idsmode) {
	    ...
	    if (OFLAG(ids[def_beam]) == 1) {
		expo[beam_nr] = expo[beam_nr] + ITM(ids[def_beam])
		...
		call sum_spec (Memr[accums[beam_nr]], wave1, wave2, 
		    W0(ids[def_beam]), WPC(ids[def_beam]), Memr[counto[beam_nr]+
		    ico[beam_nr]-1], last_len[beam_nr])

		# De-extinct spectrum
		if (extinct && !exflag) {
		    airm = AIRMASS (ids[beam_nr])
		    call de_ext_spec (Memr[accums[beam_nr]], airm, 
			W0(ids[def_beam]), WPC(ids[def_beam]), Memr[wave_tbl], 
		...
		ic = ics[beam_nr]
		exps[beam_nr] = exps[beam_nr] + ITM(ids[def_beam])
	    }
		...
		W0(ids[def_beam]), WPC(ids[def_beam]), Memr[ipc+ic-1], 
		last_len[beam_nr])
...
procedure accum_out (ids, accum, image, ncols, title, root, rec, beam_nr)
...
begin
	...
	do i = 1, MAX_NR_BEAMS {
		...
		call accum_out (ids[i], Memr[accumo[i]], image[1,i], 
